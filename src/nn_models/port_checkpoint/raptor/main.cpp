...
int main(){
    ...
    ofs << "\n" << target::rlt::save_code(target_device, target_input, std::string("_0"), true);
    ofs << "\n" << "}";
    ofs << "\n" << "namespace rl_tools::checkpoint::example::outputs{";
    ofs << "\n" << target::rlt::save_code(target_device, target_original_output, std::string("_0"), true);
    ofs << "\n" << "}";
    ...
    auto write_checkpoint = [&target_device, &target_policy, &target_input, &target_original_output](auto& actor_file, bool bare = false, bool overwrite_checkpoint_name = true){
        ...
        if (!bare){
            ...
            auto inputs_group = target::rlt::create_group(target_device, example_group, "inputs");
            target::rlt::save(target_device, target_input, inputs_group, "0");
            auto outputs_group = target::rlt::create_group(target_device, example_group, "outputs");
            target::rlt::save(target_device, target_original_output, outputs_group, "0");
        }
    };
