...
// stb_image_write declarations are provided by the optix backend (via scene.h transitive includes)
// which also defines STB_IMAGE_WRITE_IMPLEMENTATION. We undef it here to prevent any
// subsequent include from re-defining the implementation functions.
