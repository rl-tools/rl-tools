...
em++ --std=c++17 -msimd128 -O3 -s WASM=1 -I$RL_TOOLS_INCLUDE_DIR -I$JSON_INCLUDE_DIR -s EXPORTED_FUNCTIONS="$EXPORTED_FUNCTIONS" -s MODULARIZE=1 -s EXPORT_ES6=1 -s EXPORTED_RUNTIME_METHODS=UTF8ToString -s USE_ES6_IMPORT_META=0 -s ENVIRONMENT='web' --pre-js prejs.js -o $RL_TOOLS_BUILD_DIR/wasm_interface.js wasm_interface.cpp
