...
evaluate_batch_kernel(devices::CUDA<DEV_SPEC>& device, const nn::layers::dense::Layer<SPEC> layer, const typename SPEC::T* input, typename SPEC::T* output) {
    ...
    T acc = 0;
    for(TI block_reduction_i = 0; block_reduction_i < BACKPROP_TOOLS_DEVICES_CUDA_CEIL(INPUT_DIM, BLOCK_SIZE) * BLOCK_SIZE; block_reduction_i += BLOCK_SIZE){
...
//            constexpr typename devices::CUDA<DEV_SPEC>::index_t N_BLOCKS_BATCH = BACKPROP_TOOLS_DEVICES_CUDA_CEIL(BATCH_SIZE, BLOCKSIZE_BATCH);
//            constexpr typename devices::CUDA<DEV_SPEC>::index_t BLOCKSIZE_OUTPUT = 32;
//            constexpr typename devices::CUDA<DEV_SPEC>::index_t N_BLOCKS_OUTPUT = BACKPROP_TOOLS_DEVICES_CUDA_CEIL(BATCH_SIZE, BLOCKSIZE_OUTPUT);
