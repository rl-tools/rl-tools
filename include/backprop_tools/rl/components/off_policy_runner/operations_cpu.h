#include "../../../version.h"
#if (defined(BACKPROP_TOOLS_DISABLE_INCLUDEGUARDS) || !defined(BACKPROP_TOOLS_RL_COMPONENTS_OFF_POLICY_RUNNER_OPERATIONS_CPU_H)) && (BACKPROP_TOOLS_USE_THIS_VERSION == 1)
#pragma once
    ...
    void prologue(devices::CPU<DEV_SPEC>& device, rl::components::OffPolicyRunner<SPEC>& runner, RNG &rng) {
        ...
        if(NUM_THREADS > 1){
            ...
            for (TI env_i = 0; env_i < SPEC::N_ENVIRONMENTS; env_i++) {
                rngs[env_i] = backprop_tools::random::default_engine(typename DEV_SPEC::RANDOM(), base + env_i);
            }
