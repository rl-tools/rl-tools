...
namespace rl_tools{
    namespace rl::components::on_policy_runner::detail{
        ...
        RL_TOOLS_FUNCTION_PLACEMENT void prologue_env(DEVICE& device, DATASET& dataset, RUNNER& runner, typename RUNNER::TI env_i){
            using T = typename DATASET::T;
            const EpisodeEndReason reason = get(device, runner.completed_episode_reason, env_i);
            set(dataset.reset, env_i, 0, get(device, runner.reset, env_i) ? (T)1 : (T)0);
            set(device, dataset.episode_end_reason, reason == EpisodeEndReason::FORCED ? reason : EpisodeEndReason::NONE, 0, env_i);
            set(device, dataset.episode_length, reason == EpisodeEndReason::FORCED ? get(device, runner.completed_episode_length, env_i) : 0, 0, env_i);
            set(device, dataset.episode_return, reason == EpisodeEndReason::FORCED ? get(device, runner.completed_episode_return, env_i) : 0, 0, env_i);
            set(device, runner.completed_episode_reason, EpisodeEndReason::NONE, env_i);
            set(device, runner.completed_episode_length, 0, env_i);
            set(device, runner.completed_episode_return, 0, env_i);
        }
        template <typename DEVICE, typename DATASET, typename RUNNER, typename BUFFER>
        RL_TOOLS_FUNCTION_PLACEMENT void epilogue_env(DEVICE& device, DATASET& dataset, RUNNER& runner, const BUFFER& buffer, typename RUNNER::TI step_i, typename RUNNER::TI env_i){
            using T = typename DATASET::T;
            ...
            const typename RUNNER::EPISODE_T reward = get(device, buffer.rewards, env_i);
            const typename RUNNER::EPISODE_T episode_return = get(device, runner.episode_return, env_i) + reward;
            const bool terminated = get(device, buffer.terminated, env_i);
            ...
            const EpisodeEndReason reason = terminated ? EpisodeEndReason::TERMINATED : time_limit ? EpisodeEndReason::TIME_LIMIT : EpisodeEndReason::NONE;
        ...
        RL_TOOLS_FUNCTION_PLACEMENT void reset_env(DEVICE& device, RUNNER& runner, typename RUNNER::TI env_i){
            using TI = typename RUNNER::TI;
            ...
            set(device, runner.completed_episode_reason, episode_step > 0 ? EpisodeEndReason::FORCED : EpisodeEndReason::NONE, env_i);
    ...
    namespace rl::components::on_policy_runner{
        namespace detail{
            ...
            void prologue(DEVICE& device, Dataset<DATASET_SPEC>& dataset, rl::components::OnPolicyRunner<SPEC>& runner){
                using TI = typename SPEC::TI;
            ...
            template <typename DEVICE, typename DATASET_SPEC, typename SPEC, typename rl_tools::utils::typing::enable_if<DEVICE::DEVICE_ID != devices::DeviceId::CUDA, bool>::type = true>
            void epilogue(DEVICE& device, Dataset<DATASET_SPEC>& dataset, rl::components::OnPolicyRunner<SPEC>& runner, const Buffer<SPEC>& buffer, typename SPEC::TI step_i){
                using TI = typename SPEC::TI;
            ...
            void reset(DEVICE& device, rl::components::OnPolicyRunner<SPEC>& runner){
                using TI = typename SPEC::TI;
                for(TI env_i = 0; env_i < SPEC::N_ENVIRONMENTS; env_i++){
            ...
            void reset(DEVICE& device, rl::components::OnPolicyRunner<SPEC>& runner, const Tensor<MASK_SPEC>& mask){
                using TI = typename SPEC::TI;
    ...
    namespace rl::components::on_policy_runner{
        ...
        void prologue(DEVICE& device, Dataset<DATASET_SPEC>& dataset, rl::components::OnPolicyRunner<SPEC>& runner, ENVIRONMENT& environment, RNG& rng){
            detail::prologue(device, dataset, runner);
        ...
        void epilogue(DEVICE& device, Dataset<DATASET_SPEC>& dataset, rl::components::OnPolicyRunner<SPEC>& runner, Buffer<SPEC>& buffer, ENVIRONMENT& environment, RNG& rng, typename SPEC::TI step_i){
            ...
            detail::epilogue(device, dataset, runner, buffer, step_i);
        ...
        void reset(DEVICE& device, rl::components::OnPolicyRunner<SPEC>& runner, ENVIRONMENT& environment, const Tensor<MASK_SPEC>& mask, RNG& rng){
            detail::reset(device, runner, mask);
