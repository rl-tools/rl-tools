...
namespace rl_tools{
    ...
    // ======================== ReacherMemoryVisual operations ========================
    template<typename DEVICE, typename SPEC>
    RL_TOOLS_FUNCTION_PLACEMENT static void malloc(DEVICE& device, const rl::environments::ReacherMemoryVisual<SPEC>& env){}
    template<typename DEVICE, typename SPEC>
    RL_TOOLS_FUNCTION_PLACEMENT static void free(DEVICE& device, const rl::environments::ReacherMemoryVisual<SPEC>& env){}
    template<typename DEVICE, typename SPEC>
    RL_TOOLS_FUNCTION_PLACEMENT static void init(DEVICE& device, const rl::environments::ReacherMemoryVisual<SPEC>& env){}
    template<typename DEVICE, typename SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static void sample_initial_parameters(DEVICE& device, const rl::environments::ReacherMemoryVisual<SPEC>& env, typename rl::environments::ReacherMemoryVisual<SPEC>::Parameters& parameters, RNG& rng){}
    template<typename DEVICE, typename SPEC>
    RL_TOOLS_FUNCTION_PLACEMENT static void initial_parameters(DEVICE& device, const rl::environments::ReacherMemoryVisual<SPEC>& env, typename rl::environments::ReacherMemoryVisual<SPEC>::Parameters& parameters){}
    template<typename DEVICE, typename SPEC, typename STATE_SPEC>
    static void initial_state(DEVICE& device, const rl::environments::ReacherMemoryVisual<SPEC>& env, typename rl::environments::ReacherMemoryVisual<SPEC>::Parameters& parameters, typename rl::environments::reacher::StateWithStep<STATE_SPEC>& state){
        state.x = 0;
        state.y = 0;
        state.target_x = 0.5;
        state.target_y = 0.5;
    ...
    template<typename DEVICE, typename SPEC, typename STATE_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static void sample_initial_state(DEVICE& device, const rl::environments::ReacherMemoryVisual<SPEC>& env, typename rl::environments::ReacherMemoryVisual<SPEC>::Parameters& parameters, typename rl::environments::reacher::StateWithStep<STATE_SPEC>& state, RNG& rng){
        using T = typename SPEC::T;
        using PARAMS = typename SPEC::PARAMETERS;
        state.x = random::uniform_real_distribution(device.random, -PARAMS::ARENA_SIZE, PARAMS::ARENA_SIZE, rng);
        state.y = random::uniform_real_distribution(device.random, -PARAMS::ARENA_SIZE, PARAMS::ARENA_SIZE, rng);
    ...
    template<typename DEVICE, typename SPEC, typename STATE_SPEC, typename ACTION_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT typename SPEC::T step(DEVICE& device, const rl::environments::ReacherMemoryVisual<SPEC>& env, typename rl::environments::ReacherMemoryVisual<SPEC>::Parameters& parameters, const typename rl::environments::reacher::StateWithStep<STATE_SPEC>& state, const Matrix<ACTION_SPEC>& action, typename rl::environments::reacher::StateWithStep<STATE_SPEC>& next_state, RNG& rng){
        static_assert(ACTION_SPEC::ROWS == 1);
        static_assert(ACTION_SPEC::COLS == 2);
        using T = typename SPEC::T;
        using TI = typename SPEC::TI;
    ...
    template<typename DEVICE, typename SPEC, typename ACTION_SPEC, typename STATE_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static typename SPEC::T reward(DEVICE& device, const rl::environments::ReacherMemoryVisual<SPEC>& env, typename rl::environments::ReacherMemoryVisual<SPEC>::Parameters& parameters, const typename rl::environments::reacher::StateWithStep<STATE_SPEC>& state, const Matrix<ACTION_SPEC>& action, const typename rl::environments::reacher::StateWithStep<STATE_SPEC>& next_state, RNG& rng){
        using T = typename SPEC::T;
        T dx = next_state.x - next_state.target_x;
        T dy = next_state.y - next_state.target_y;
        T distance = math::sqrt(device.math, dx * dx + dy * dy);
    ...
    template<typename DEVICE, typename SPEC, typename STATE_SPEC, typename OBS_TYPE_SPEC, typename OBS_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static void observe(DEVICE& device, const rl::environments::ReacherMemoryVisual<SPEC>& env, const typename rl::environments::ReacherMemoryVisual<SPEC>::Parameters& parameters, const typename rl::environments::reacher::StateWithStep<STATE_SPEC>& state, const typename rl::environments::reacher::ObservationDense<OBS_TYPE_SPEC>&, Matrix<OBS_SPEC>& observation, RNG& rng){
        static_assert(OBS_SPEC::ROWS == 1);
        static_assert(OBS_SPEC::COLS == 4);
        set(observation, 0, 0, state.x);
        set(observation, 0, 1, state.y);
    ...
    template<typename DEVICE, typename SPEC, typename STATE_SPEC, typename OBS_TYPE_TI, OBS_TYPE_TI OBS_HEIGHT, OBS_TYPE_TI OBS_WIDTH, typename OBS_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static void observe(DEVICE& device, const rl::environments::ReacherMemoryVisual<SPEC>& env, const typename rl::environments::ReacherMemoryVisual<SPEC>::Parameters& parameters, const typename rl::environments::reacher::StateWithStep<STATE_SPEC>& state, const typename rl::environments::reacher::ObservationImage<OBS_TYPE_TI, OBS_HEIGHT, OBS_WIDTH>&, Matrix<OBS_SPEC>& observation, RNG& rng){
        using T = typename SPEC::T;
        using TI = typename SPEC::TI;
        using PARAMS = typename SPEC::PARAMETERS;
        static_assert(OBS_SPEC::ROWS == 1);
        ...
        T target_px = (state.target_x + PARAMS::ARENA_SIZE) / ((T)2 * PARAMS::ARENA_SIZE) * H;
        T target_py = (state.target_y + PARAMS::ARENA_SIZE) / ((T)2 * PARAMS::ARENA_SIZE) * W;
        bool show_target = (state.step == 0);
        for(TI h = 0; h < H; h++){
            for(TI w = 0; w < W; w++){
                T dh_agent = (T)h + (T)0.5 - agent_px;
                T dw_agent = (T)w + (T)0.5 - agent_py;
                T agent_intensity = math::exp(device.math, -(dh_agent * dh_agent + dw_agent * dw_agent) / sigma_sq);
                T target_intensity = (T)0;
                if(show_target){
                    T dh_target = (T)h + (T)0.5 - target_px;
                    T dw_target = (T)w + (T)0.5 - target_py;
                    target_intensity = math::exp(device.math, -(dh_target * dh_target + dw_target * dw_target) / sigma_sq);
                }
                TI base = h * W * C + w * C;
                set(observation, 0, base + 0, target_intensity);
                set(observation, 0, base + 1, (T)0);
    ...
    template<typename DEVICE, typename SPEC, typename STATE_SPEC, typename OBS_TYPE_TI, OBS_TYPE_TI OBS_HEIGHT, OBS_TYPE_TI OBS_WIDTH, typename OBS_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static void observe(DEVICE& device, const rl::environments::ReacherMemoryVisual<SPEC>& env, const typename rl::environments::ReacherMemoryVisual<SPEC>::Parameters& parameters, const typename rl::environments::reacher::StateWithStep<STATE_SPEC>& state, const typename rl::environments::reacher::ObservationImageFlat<OBS_TYPE_TI, OBS_HEIGHT, OBS_WIDTH>&, Matrix<OBS_SPEC>& observation, RNG& rng){
        using T = typename SPEC::T;
        using TI = typename SPEC::TI;
        using PARAMS = typename SPEC::PARAMETERS;
        static_assert(OBS_SPEC::ROWS == 1);
    ...
    template<typename DEVICE, typename SPEC, typename STATE_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static bool terminated(DEVICE& device, const rl::environments::ReacherMemoryVisual<SPEC>& env, typename rl::environments::ReacherMemoryVisual<SPEC>::Parameters& parameters, const typename rl::environments::reacher::StateWithStep<STATE_SPEC> state, RNG& rng){
        using T = typename SPEC::T;
        using PARAMS = typename SPEC::PARAMETERS;
        T dx = state.x - state.target_x;
        T dy = state.y - state.target_y;
    ...
    template<typename DEVICE, typename STATE_SPEC>
    RL_TOOLS_FUNCTION_PLACEMENT typename STATE_SPEC::T abs_diff(DEVICE& device, const rl::environments::reacher::StateWithStep<STATE_SPEC>& s1, const rl::environments::reacher::StateWithStep<STATE_SPEC>& s2){
        using T = typename STATE_SPEC::T;
        T acc = 0;
        acc += math::abs(device.math, s1.x - s2.x);
        acc += math::abs(device.math, s1.y - s2.y);
    ...
    // ======================== ReacherVisualMemoryHard operations ========================
    template<typename DEVICE, typename SPEC>
    RL_TOOLS_FUNCTION_PLACEMENT static void malloc(DEVICE& device, const rl::environments::ReacherVisualMemoryHard<SPEC>& env){}
    template<typename DEVICE, typename SPEC>
    RL_TOOLS_FUNCTION_PLACEMENT static void free(DEVICE& device, const rl::environments::ReacherVisualMemoryHard<SPEC>& env){}
    template<typename DEVICE, typename SPEC>
    RL_TOOLS_FUNCTION_PLACEMENT static void init(DEVICE& device, const rl::environments::ReacherVisualMemoryHard<SPEC>& env){}
    template<typename DEVICE, typename SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static void sample_initial_parameters(DEVICE& device, const rl::environments::ReacherVisualMemoryHard<SPEC>& env, typename rl::environments::ReacherVisualMemoryHard<SPEC>::Parameters& parameters, RNG& rng){}
    template<typename DEVICE, typename SPEC>
    RL_TOOLS_FUNCTION_PLACEMENT static void initial_parameters(DEVICE& device, const rl::environments::ReacherVisualMemoryHard<SPEC>& env, typename rl::environments::ReacherVisualMemoryHard<SPEC>::Parameters& parameters){}
    template<typename DEVICE, typename SPEC, typename STATE_SPEC>
    static void initial_state(DEVICE& device, const rl::environments::ReacherVisualMemoryHard<SPEC>& env, typename rl::environments::ReacherVisualMemoryHard<SPEC>::Parameters& parameters, typename rl::environments::reacher::StateSequentialTargets<STATE_SPEC>& state){
        state.x = 0;
        state.y = 0;
        state.target1_x = 0.5;
        state.target1_y = 0.5;
    ...
    template<typename DEVICE, typename SPEC, typename STATE_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static void sample_initial_state(DEVICE& device, const rl::environments::ReacherVisualMemoryHard<SPEC>& env, typename rl::environments::ReacherVisualMemoryHard<SPEC>::Parameters& parameters, typename rl::environments::reacher::StateSequentialTargets<STATE_SPEC>& state, RNG& rng){
        using T = typename SPEC::T;
        using PARAMS = typename SPEC::PARAMETERS;
        state.x = random::uniform_real_distribution(device.random, -PARAMS::ARENA_SIZE, PARAMS::ARENA_SIZE, rng);
        state.y = random::uniform_real_distribution(device.random, -PARAMS::ARENA_SIZE, PARAMS::ARENA_SIZE, rng);
    ...
    template<typename DEVICE, typename SPEC, typename STATE_SPEC, typename ACTION_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT typename SPEC::T step(DEVICE& device, const rl::environments::ReacherVisualMemoryHard<SPEC>& env, typename rl::environments::ReacherVisualMemoryHard<SPEC>::Parameters& parameters, const typename rl::environments::reacher::StateSequentialTargets<STATE_SPEC>& state, const Matrix<ACTION_SPEC>& action, typename rl::environments::reacher::StateSequentialTargets<STATE_SPEC>& next_state, RNG& rng){
        static_assert(ACTION_SPEC::ROWS == 1);
        static_assert(ACTION_SPEC::COLS == 2);
        using T = typename SPEC::T;
        using TI = typename SPEC::TI;
    ...
    template<typename DEVICE, typename SPEC, typename ACTION_SPEC, typename STATE_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static typename SPEC::T reward(DEVICE& device, const rl::environments::ReacherVisualMemoryHard<SPEC>& env, typename rl::environments::ReacherVisualMemoryHard<SPEC>::Parameters& parameters, const typename rl::environments::reacher::StateSequentialTargets<STATE_SPEC>& state, const Matrix<ACTION_SPEC>& action, const typename rl::environments::reacher::StateSequentialTargets<STATE_SPEC>& next_state, RNG& rng){
        using T = typename SPEC::T;
        T dx, dy;
    ...
    template<typename DEVICE, typename SPEC, typename STATE_SPEC, typename OBS_TYPE_SPEC, typename OBS_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static void observe(DEVICE& device, const rl::environments::ReacherVisualMemoryHard<SPEC>& env, const typename rl::environments::ReacherVisualMemoryHard<SPEC>::Parameters& parameters, const typename rl::environments::reacher::StateSequentialTargets<STATE_SPEC>& state, const typename rl::environments::reacher::ObservationDenseSequentialTargets<OBS_TYPE_SPEC>&, Matrix<OBS_SPEC>& observation, RNG& rng){
        static_assert(OBS_SPEC::ROWS == 1);
    ...
    template<typename DEVICE, typename SPEC, typename STATE_SPEC, typename OBS_TYPE_TI, OBS_TYPE_TI OBS_HEIGHT, OBS_TYPE_TI OBS_WIDTH, typename OBS_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static void observe(DEVICE& device, const rl::environments::ReacherVisualMemoryHard<SPEC>& env, const typename rl::environments::ReacherVisualMemoryHard<SPEC>::Parameters& parameters, const typename rl::environments::reacher::StateSequentialTargets<STATE_SPEC>& state, const typename rl::environments::reacher::ObservationImage<OBS_TYPE_TI, OBS_HEIGHT, OBS_WIDTH>&, Matrix<OBS_SPEC>& observation, RNG& rng){
        using T = typename SPEC::T;
        using TI = typename SPEC::TI;
        using PARAMS = typename SPEC::PARAMETERS;
        static_assert(OBS_SPEC::ROWS == 1);
        ...
        T target2_px = (state.target2_x + PARAMS::ARENA_SIZE) / ((T)2 * PARAMS::ARENA_SIZE) * H;
        T target2_py = (state.target2_y + PARAMS::ARENA_SIZE) / ((T)2 * PARAMS::ARENA_SIZE) * W;
    ...
    template<typename DEVICE, typename SPEC, typename STATE_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static bool terminated(DEVICE& device, const rl::environments::ReacherVisualMemoryHard<SPEC>& env, typename rl::environments::ReacherVisualMemoryHard<SPEC>::Parameters& parameters, const typename rl::environments::reacher::StateSequentialTargets<STATE_SPEC> state, RNG& rng){
        using T = typename SPEC::T;
        using PARAMS = typename SPEC::PARAMETERS;
        if(state.current_target == 0){
            return false;
