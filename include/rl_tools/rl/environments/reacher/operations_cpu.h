...
export async function render(ui_state, parameters, state, action) {
    ...
    const arenaSize = parameters.arena_size;
    const targetRadius = parameters.target_radius;
    const imgH = parameters.image_height || 32;
    const imgW = parameters.image_width || 32;
    ...
    // Minimap: render the observation image pixel by pixel
    ...
    for (let row = 0; row < imgH; row++) {
        for (let col = 0; col < imgW; col++) {
            ...
            const target_intensity = Math.exp(-(dh_target * dh_target + dw_target * dw_target) / sigma_sq);
    ...
    ctx.fillText(imgW + 'x' + imgH + ' obs', mmX, mmY + mmH + w * 0.04);
}
        )RL_TOOLS_LITERAL";
        return ui;
    }
...
export async function render(ui_state, parameters, state, action) {
    ...
    const arenaSize = parameters.arena_size;
    const targetRadius = parameters.target_radius;
    const imgH = parameters.image_height || 16;
    const imgW = parameters.image_width || 16;
    const minimapSize = Math.min(w, h) * 0.3;
    const mainSize = w - minimapSize - w * 0.05;
    const margin = w * 0.03;
    const drawSize = mainSize - 2 * margin;
    ...
    ctx.fillStyle = '#666';
    ctx.font = Math.round(w * 0.03) + 'px monospace';
    ctx.fillText(imgW + 'x' + imgH + ' obs (step=' + state.step + ')', mmX, mmY + mmH + w * 0.04);
}
        )RL_TOOLS_LITERAL";
        return ui;
    }
