...
namespace rl_tools::rl::environments::car{
    ...
    std::string json(const DEVICE& device, const rl::environments::Car<SPEC>, const rl::environments::car::Parameters<T>& parameters){
        nlohmann::json j;
        j["lf"]  = parameters.lf;
        j["lr"]  = parameters.lr;
        j["dt"]  = parameters.dt;
    ...
    std::string json(const DEVICE& device, const rl::environments::CarTrack<SPEC>, const rl::environments::car::ParametersTrack<T, TI, T_HEIGHT, T_WIDTH, T_TRACK_SCALE>& parameters){
