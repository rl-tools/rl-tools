...
namespace rl_tools{
    namespace rl::environments::l2f{
        ...
        inline bool _is_nan(DEVICE& device, rl::environments::l2f::StateBase<STATE_SPEC>& state){
            bool nan = false;
        ...
        template<typename DEVICE, typename STATE_SPEC>
        inline bool _is_nan(DEVICE& device, rl::environments::l2f::StatePoseErrorIntegral<STATE_SPEC>& state){
        ...
        template<typename DEVICE, typename STATE_SPEC>
        inline bool _is_nan(DEVICE& device, rl::environments::l2f::StateRandomForce<STATE_SPEC>& state){
            is_nan(device, static_cast<typename STATE_SPEC::NEXT_COMPONENT&>(state));
            bool nan = false;
        ...
        template<typename DEVICE, typename STATE_SPEC>
        inline bool _is_nan(DEVICE& device, rl::environments::l2f::StateRotors<STATE_SPEC>& state){
            is_nan(device, static_cast<typename STATE_SPEC::NEXT_COMPONENT&>(state));
            bool nan = false;
            for(typename DEVICE::index_t i = 0; i < 4; i++){
        ...
        template<typename DEVICE, typename STATE_SPEC>
        inline bool _is_nan(DEVICE& device, rl::environments::l2f::StateRotorsHistory<STATE_SPEC>& state){
