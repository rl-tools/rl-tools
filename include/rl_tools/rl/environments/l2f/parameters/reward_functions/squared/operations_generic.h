...
namespace rl_tools::rl::environments::l2f::parameters::reward_functions{
    ...
    RL_TOOLS_FUNCTION_PLACEMENT void _reward_components(DEVICE& device, const Multirotor<SPEC>& env, const PARAMETERS& parameters, const Squared<T, T_ACTION_DIM>& reward_parameters, const StateBase<STATE_SPEC>& state_dispatch, const STATE& state, const Matrix<ACTION_SPEC>& action,  const STATE& next_state, typename Squared<T, T_ACTION_DIM>::Components& components, RNG& rng){
...
//        components.orientation_cost = 1 - state.orientation[0] * state.orientation[0]; //math::abs(device.math, 2 * math::acos(device.math, quaternion_w));
        ...
        for(TI action_i = 0; action_i < ACTION_DIM; action_i++){
            ...
            if constexpr(SPEC::STATIC_PARAMETERS::ACTION_INTERFACE == rl_tools::rl::environments::l2f::parameters::ActionInterface::CTBR){
                if(action_i == 0){
                    T collective = (normalized_action + (T)1) / (T)2;
                    ...
                    action_diff = collective - parameters.dynamics.hovering_throttle_relative;
                }
                else{
                    action_diff = normalized_action;
                }
            }
            else{
    ...
    template<typename DEVICE, typename SPEC, typename PARAMETERS, typename STATE_SPEC, typename STATE, typename ACTION_SPEC, typename T, unsigned T_ACTION_DIM, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT void _reward_components(DEVICE& device, const Multirotor<SPEC>& env, const PARAMETERS& parameters, const Squared<T, T_ACTION_DIM>& reward_parameters, const StateLastAction<STATE_SPEC>& state_dispatch, const STATE& state, const Matrix<ACTION_SPEC>& action,  const STATE& next_state, typename Squared<T, T_ACTION_DIM>::Components& components, RNG& rng) {
        using TI = typename DEVICE::index_t;
        constexpr TI ACTION_DIM = rl::environments::Multirotor<SPEC>::ACTION_DIM;
        static_assert(ACTION_DIM == T_ACTION_DIM);
    ...
    template<typename DEVICE, typename SPEC, typename PARAMETERS, typename ACTION_SPEC, typename STATE, typename T, unsigned T_ACTION_DIM, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT typename SPEC::T reward(DEVICE& device, const Multirotor<SPEC>& env, const PARAMETERS& parameters, const Squared<T, T_ACTION_DIM>& reward_parameters, const STATE& state, const Matrix<ACTION_SPEC>& action,  const STATE& next_state, RNG& rng){
        typename Squared<T, T_ACTION_DIM>::Components components;
    ...
    template<typename DEVICE, typename T, unsigned T_ACTION_DIM>
    RL_TOOLS_FUNCTION_PLACEMENT constexpr auto name(DEVICE& device, const Squared<T, T_ACTION_DIM>& reward_parameters){
        return "squared";
    }
}
