#include "../version.h"
#if (defined(RL_TOOLS_DISABLE_INCLUDE_GUARDS) || !defined(RL_TOOLS_LOGGING_OPERATIONS_ARDUINO_H)) && (RL_TOOLS_USE_THIS_VERSION == 1)
#pragma once
#define RL_TOOLS_LOGGING_OPERATIONS_ARDUINO_H
...
namespace rl_tools{
    ...
    void add_text(DEVICE& device, devices::logging::ARDUINO& logger, const TOPIC, const TEXT){ /* noop */ }
    template <typename DEVICE, typename TOPIC, typename TEXT, typename CADENCE>
    void add_text(DEVICE& device, devices::logging::ARDUINO& logger, const TOPIC, const TEXT, const CADENCE){ /* noop */ }
    template <typename DEVICE, typename HPARAMS, typename METRICS>
    void add_hparams(DEVICE& device, devices::logging::ARDUINO& logger, const HPARAMS&, const METRICS&){ /* noop */ }
}
