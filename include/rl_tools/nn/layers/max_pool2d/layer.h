#include "../../../version.h"
#if (defined(RL_TOOLS_DISABLE_INCLUDE_GUARDS) || !defined(RL_TOOLS_NN_LAYERS_MAX_POOL2D_LAYER_H)) && (RL_TOOLS_USE_THIS_VERSION == 1)
#pragma once
#define RL_TOOLS_NN_LAYERS_MAX_POOL2D_LAYER_H
...
namespace rl_tools::nn::layers::max_pool2d {

    template<typename T_TYPE_POLICY, typename T_TI,
             T_TI T_KERNEL_HEIGHT, T_TI T_KERNEL_WIDTH = T_KERNEL_HEIGHT,
             T_TI T_STRIDE_H = T_KERNEL_HEIGHT, T_TI T_STRIDE_W = T_KERNEL_WIDTH,
    ...
    struct Specification: T_CAPABILITY, T_CONFIG{
        ...
        static_assert(length(INPUT_SHAPE{}) >= 4, "MaxPool2d input shape must have at least 4 dimensions (...BATCH x H x W x C)");
        ...
        struct OUTPUT_SHAPE_FACTORY{
            ...
            using SHAPE = tensor::Replace<
                tensor::Replace<NEW_INPUT_SHAPE, NEW_OW, length(NEW_INPUT_SHAPE{})-2>,
    ...
    struct LayerForward {
        ...
        using Buffer = max_pool2d::Buffer;
        template<bool DYNAMIC_ALLOCATION=true>
        using State = max_pool2d::State;
    };
    ...
    struct BindConfiguration{
        ...
        using Layer = nn::layers::max_pool2d::Layer<CONFIG, CAPABILITY, INPUT_SHAPE>;
    };
}
