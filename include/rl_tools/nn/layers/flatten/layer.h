#include "../../../version.h"
#if (defined(RL_TOOLS_DISABLE_INCLUDE_GUARDS) || !defined(RL_TOOLS_NN_LAYERS_FLATTEN_LAYER_H)) && (RL_TOOLS_USE_THIS_VERSION == 1)
#pragma once
#define RL_TOOLS_NN_LAYERS_FLATTEN_LAYER_H
...
namespace rl_tools::nn::layers::flatten {
    ...
    struct Specification: T_CAPABILITY, T_CONFIG{
        ...
        static_assert(length(INPUT_SHAPE{}) >= 4, "Flatten input shape must have at least 4 dimensions (...BATCH x H x W x C)");
        ...
        static constexpr TI OUTPUT_DIM = INPUT_HEIGHT * INPUT_WIDTH * INPUT_CHANNELS;
        ...
        // Output shape: (...BATCH, H*W*C) — flatten spatial dims and channels into one
        ...
        struct OUTPUT_SHAPE_FACTORY{
            ...
            static_assert(NEW_H == INPUT_HEIGHT && NEW_W == INPUT_WIDTH && NEW_C == INPUT_CHANNELS);
            using NEW_BATCH_SHAPE = tensor::PopBack<tensor::PopBack<tensor::PopBack<NEW_INPUT_SHAPE>>>;
            using SHAPE = tensor::Append<NEW_BATCH_SHAPE, NEW_H * NEW_W * NEW_C>;
        };
    ...
    constexpr bool check_input_output =
        ...
        get_last(typename OUTPUT_SPEC::SHAPE{}) == LAYER_SPEC::OUTPUT_DIM;
    ...
    struct LayerForward {
        ...
        using Buffer = flatten::Buffer;
        template<bool DYNAMIC_ALLOCATION=true>
        using State = flatten::State;
    };
    ...
    struct LayerGradient: public LayerBackward<SPEC>{
        ...
        using TI = typename SPEC::TI;
        using OUTPUT_CONTAINER_SHAPE = tensor::Shape<TI, SPEC::INTERNAL_BATCH_SIZE, SPEC::OUTPUT_DIM>;
    ...
    struct BindConfiguration{
        ...
        using Layer = nn::layers::flatten::Layer<CONFIG, CAPABILITY, INPUT_SHAPE>;
    };
}
