...
namespace rl_tools{
        ...
        void update_kernel(devices::CUDA<DEV_SPEC> device, nn::layers::dense::LayerGradient<SPEC> layer, nn::optimizers::Adam<PARAMETERS> optimizer) {
            ...
            if(input_i < INPUT_DIM && output_i < OUTPUT_DIM){
                if(input_i == 0){
                    ...
                    TO d_bias_second_order_moment = optimizer_parameters.beta_2 * (TO)get(device, layer.biases.gradient_second_order_moment, output_i) + (1 - optimizer_parameters.beta_2) * d_bias * d_bias;
                ...
                }
                {
                    ...
                    TO d_weight_second_order_moment = optimizer_parameters.beta_2 * (TO)get(device, layer.weights.gradient_second_order_moment, output_i, input_i) + (1 - optimizer_parameters.beta_2) * d_weight * d_weight;
    ...
    RL_TOOLS_FUNCTION_PLACEMENT void evaluate(devices::CUDA<DEV_SPEC>& device, const nn::layers::dense::LayerForward<LAYER_SPEC>& layer, const Matrix<INPUT_SPEC>& input, Matrix<OUTPUT_SPEC>& output, nn::layers::dense::Buffer&, RNG& rng, const Mode<MODE>& mode = Mode<mode::Default<>>{}){
        ...
        using TI = typename DEVICE::index_t;
        {
            ...
            else if constexpr(utils::typing::is_same_v<T, double>){
                constexpr T alpha = 1, beta = 1;
