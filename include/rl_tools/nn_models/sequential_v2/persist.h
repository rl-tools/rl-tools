...
namespace rl_tools{
        ...
        void save(DEVICE& device, nn_models::sequential_v2::Module<SPEC>& model, HighFive::Group group, typename DEVICE::index_t layer_i = 0) {
            if(layer_i == 0){
    ...
    template<typename DEVICE, typename SPEC>
    void load(DEVICE& device, nn_models::sequential_v2::Module<SPEC>& model, HighFive::Group group, typename DEVICE::index_t layer_i = 0) {
        if(layer_i == 0){
