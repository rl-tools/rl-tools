...
#include "tensor_operations_generic.h"
...
namespace rl_tools{
    namespace dyn{
        ...
        RL_TOOLS_FUNCTION_PLACEMENT void propagate_parallel_output(Layer<TI>& layer){
            ...
            if(p == nullptr || p->num_branches == 0){ layer.output_size = 0; return; }
            TI num_branches = p->num_branches;
            TI rank = layer.children[0].output_rank;
            TI concat_last = 0;
            TI total_size = 0;
            for(TI i = 0; i < num_branches; i++){
                concat_last += layer.children[i].output_shape[rank - 1];
                total_size += layer.children[i].output_size;
            }
            layer.output_rank = rank;
            layer.output_size = total_size;
            for(TI i = 0; i < rank - 1; i++) layer.output_shape[i] = layer.children[0].output_shape[i];
            layer.output_shape[rank - 1] = concat_last;
            if(p->has_head){
                TI head_idx = num_branches;
                propagate_shapes(layer.children[head_idx], layer.output_shape, layer.output_rank);
                layer.output_rank = layer.children[head_idx].output_rank;
                layer.output_size = layer.children[head_idx].output_size;
                for(TI i = 0; i < layer.output_rank; i++) layer.output_shape[i] = layer.children[head_idx].output_shape[i];
            }
        }
        ...
        RL_TOOLS_FUNCTION_PLACEMENT void propagate_shapes(Layer<TI>& layer, const TensorTuple<TI>& inputs){
            if(layer.type == LayerType::PARALLEL && layer.num_children >= 2){
                auto* p = layer.data ? &layer.template as<const layers::Parallel<TI>>() : nullptr;
                TI num_branches = p ? p->num_branches : 0;
                for(TI i = 0; i < num_branches && i < inputs.num_tensors; i++){
        ...
        RL_TOOLS_FUNCTION_PLACEMENT TI max_output_size(const Layer<TI>& layer){
            ...
            if(layer.type == LayerType::PARALLEL){
                auto* p = layer.data ? &layer.template as<const layers::Parallel<TI>>() : nullptr;
                TI num_branches = p ? p->num_branches : 0;
                TI concat = 0;
                for(TI i = 0; i < num_branches; i++) concat += layer.children[i].output_size;
            ...
            if(input.type == Type::FLOAT32 && layer.weights.type == Type::FLOAT32 && layer.biases.type == Type::FLOAT32 && output.type == Type::FLOAT32){
                ...
                for(TI b = 0; b < batch_size; b++)
                    for(TI o = 0; o < layer.output_dim; o++){
                        ...
                        set(device, output, apply_activation(device.math, layer.activation_function, acc), b * layer.output_dim + o);
                    }
            }
        }
        template <typename DEVICE, typename TI>
        RL_TOOLS_FUNCTION_PLACEMENT void evaluate_step_gru(DEVICE& device, const layers::GRU<TI>& layer, const Tensor<TensorSpecification<TI>>& input, state::GRU<TI>& gru_state, Tensor<TensorSpecification<TI>>& scratch){
            ...
            if(input.type == Type::FLOAT32 && layer.weights_input.type == Type::FLOAT32 && layer.biases_input.type == Type::FLOAT32 && layer.weights_hidden.type == Type::FLOAT32 && layer.biases_hidden.type == Type::FLOAT32 && layer.initial_hidden_state.type == Type::FLOAT32 && gru_state.hidden.type == Type::FLOAT32 && scratch.type == Type::FLOAT32){
                ...
                if(!gru_state.initialized){
                    for(TI b = 0; b < batch_size; b++) for(TI h = 0; h < hidden_dim; h++) set(device, gru_state.hidden, get(device, layer.initial_hidden_state, h), b*hidden_dim+h);
                    gru_state.initialized = true;
                }
                for(TI b = 0; b < batch_size; b++){
                    for(TI g = 0; g < 2*hidden_dim; g++){
                        ...
                        set(device, scratch, 1.0f / (1.0f + math::exp(device.math, -(wh+wi))), b*2*hidden_dim+g);
                    }
                    for(TI h = 0; h < hidden_dim; h++){
                        TI g = 2*hidden_dim+h;
                        ...
                        set(device, scratch, z * get(device, gru_state.hidden, b*hidden_dim+h) + (1.0f-z) * n, b*2*hidden_dim+h);
                    }
                    for(TI h = 0; h < hidden_dim; h++) set(device, gru_state.hidden, get(device, scratch, b*2*hidden_dim+h), b*hidden_dim+h);
                }
            }
        }
        template <typename DEVICE, typename TI>
                    ...
                    for(TI ohi = 0; ohi < oh; ohi++)
                        for(TI owi = 0; owi < ow; owi++)
                            for(TI oc = 0; oc < layer.output_channels; oc++){
                                ...
                                set(device, output, acc, ((bi*oh+ohi)*ow+owi)*layer.output_channels+oc);
                            }
                if(layer.normalization == layers::Conv2d<TI>::Normalization::BATCH_NORM){
                    float eps = 1e-5f;
                    for(TI i = 0; i < output.size(); i++){
                        TI oc = i % layer.output_channels;
                        set(device, output, get(device, layer.gamma, oc) * (get(device, output, i) - get(device, layer.running_mean, oc)) / math::sqrt(device.math, get(device, layer.running_var, oc) + eps) + get(device, layer.beta, oc), i);
                    }
                }
                if(layer.activation_function != ActivationFunction::IDENTITY)
                    for(TI i = 0; i < output.size(); i++) set(device, output, apply_activation(device.math, layer.activation_function, get(device, output, i)), i);
            }
        }
        template <typename DEVICE, typename TI>
                ...
                for(TI ohi = 0; ohi < oh; ohi++)
                    for(TI owi = 0; owi < ow; owi++)
                        for(TI c = 0; c < ch; c++){
                            ...
                            set(device, output, mx, ((bi*oh+ohi)*ow+owi)*ch+c);
                        }
        }
    }
    ...
    namespace dyn{
        ...
        RL_TOOLS_FUNCTION_PLACEMENT void evaluate_gru(DEVICE& device, const layers::GRU<TI>& gru, const Layer<TI>& layer, const Tensor<TensorSpecification<TI>>& input, Tensor<TensorSpecification<TI>>& output, Buffer<TI>& buffer){
            ...
            if(input.rank == 2){
                ...
                for(TI b = 0; b < batch; b++) for(TI h = 0; h < hidden_dim; h++) set(device, output, get(device, tmp_state.hidden, b*hidden_dim+h), b*hidden_dim+h);
            }
            else{
                ...
                for(TI t = 0; t < seq_len; t++){
                    ...
                    for(TI b = 0; b < batch; b++) for(TI h = 0; h < hidden_dim; h++) set(device, output, get(device, tmp_state.hidden, b*hidden_dim+h), (t*batch+b)*hidden_dim+h);
                }
            }
        }
    }
    ...
    RL_TOOLS_FUNCTION_PLACEMENT bool evaluate(DEVICE& device, const dyn::Layer<TI>& layer, const dyn::Tensor<dyn::TensorSpecification<TI>>& input, dyn::Tensor<dyn::TensorSpecification<TI>>& output, dyn::Buffer<TI>& buffer){
        ...
        switch(layer.type){
            ...
            case dyn::LayerType::SAMPLE_AND_SQUASH: {
                ...
                for(TI b = 0; b < batch; b++) for(TI i = 0; i < half; i++) rl_tools::set(device, output, math::tanh(device.math, rl_tools::get(device, input, b*last+i)), b*half+i);
                return true;
            }
            case dyn::LayerType::STANDARDIZE: {
                ...
                if(input.type == dyn::Type::FLOAT32 && s.mean.type == dyn::Type::FLOAT32 && s.precision.type == dyn::Type::FLOAT32 && output.type == dyn::Type::FLOAT32){
                    ...
                    for(TI b = 0; b < batch; b++) for(TI i = 0; i < s.dim; i++) rl_tools::set(device, output, (rl_tools::get(device, input, b*s.dim+i) - rl_tools::get(device, s.mean, i)) * rl_tools::get(device, s.precision, i), b*s.dim+i);
                }
                return true;
            }
            ...
            case dyn::LayerType::AVG_POOL2D: {
                ...
                for(TI b = 0; b < batch; b++) for(TI c = 0; c < ch; c++){
                    float sum = 0; for(TI hi = 0; hi < h; hi++) for(TI wi = 0; wi < w; wi++) sum += rl_tools::get(device, input, ((b*h+hi)*w+wi)*ch+c);
                    rl_tools::set(device, output, sum*scale, b*ch+c);
                }
                return true;
            }
            ...
            case dyn::LayerType::RESNET_BLOCK: {
                ...
                for(TI i = 0; i < output.size(); i++){
                    float val = rl_tools::get(device, output, i) + rl_tools::get(device, shortcut, i);
                    rl_tools::set(device, output, val > 0 ? val : 0, i);
                }
                return true;
            }
    ...
    RL_TOOLS_FUNCTION_PLACEMENT bool evaluate(DEVICE& device, const dyn::Layer<TI>& layer, const dyn::TensorTuple<TI>& inputs, dyn::Tensor<dyn::TensorSpecification<TI>>& output, dyn::Buffer<TI>& buffer){
        if(layer.type != dyn::LayerType::PARALLEL){
            if(inputs.num_tensors > 0) return rl_tools::evaluate(device, layer, inputs.tensors[0], output, buffer);
            return false;
        }
        auto* p = layer.data ? &layer.template as<const dyn::layers::Parallel<TI>>() : nullptr;
        if(p == nullptr || p->num_branches == 0) return false;
        TI num_branches = p->num_branches;
        if(layer.output_size == 0) return false;
        if(output.capacity < layer.output_size) return false;
        dyn::set_shape(output, layer.output_rank, layer.output_shape);
        ...
        dyn::Tensor<dyn::TensorSpecification<TI>>& concat_target = p->has_head ? buffer.tick : output;
        ...
        if(p->has_head) return rl_tools::evaluate(device, layer.children[num_branches], buffer.tick, output, buffer);
        return true;
    }
    ...
    RL_TOOLS_FUNCTION_PLACEMENT bool evaluate_step(DEVICE& device, const dyn::Layer<TI>& layer, const dyn::Tensor<dyn::TensorSpecification<TI>>& input, dyn::State<TI>& state, dyn::Tensor<dyn::TensorSpecification<TI>>& output, dyn::Buffer<TI>& buffer){
        ...
        switch(layer.type){
            case dyn::LayerType::GRU: {
                ...
                for(TI b = 0; b < batch; b++) for(TI h = 0; h < g.hidden_dim; h++) rl_tools::set(device, output, rl_tools::get(device, gs->hidden, b*g.hidden_dim+h), b*g.hidden_dim+h);
                return true;
            }
            case dyn::LayerType::SEQUENTIAL: case dyn::LayerType::MLP: {
                auto* cs = reinterpret_cast<dyn::state::Composite<TI>*>(state.data);
