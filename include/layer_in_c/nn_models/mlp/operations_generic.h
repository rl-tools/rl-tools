...
namespace layer_in_c {
    ...
    FUNCTION_PLACEMENT void evaluate(nn_models::mlp::NeuralNetwork<devices::Generic, SPEC>& network, const typename SPEC::T input[SPEC::LAYER_1::INPUT_DIM], typename SPEC::T output[SPEC::OUTPUT_LAYER::OUTPUT_DIM]){
        ...
        for (size_t layer_i = 0; layer_i < SPEC::STRUCTURE_SPEC::NUM_HIDDEN_LAYERS; layer_i++){
            if(layer_i % 2 == 0){
        ...
        if constexpr(SPEC::STRUCTURE_SPEC::NUM_HIDDEN_LAYERS % 2 == 0){
    ...
    FUNCTION_PLACEMENT void zero_gradient(nn_models::mlp::NeuralNetwork<devices::Generic, SPEC>& network) {
        ...
        for(size_t i = 0; i < SPEC::STRUCTURE_SPEC::NUM_HIDDEN_LAYERS; i++){
...
//        backward(network.layer_1     , d_layer_1_output, d_input);
        ...
        for (size_t layer_i_plus_one = SPEC::STRUCTURE_SPEC::NUM_HIDDEN_LAYERS; layer_i_plus_one > 0; layer_i_plus_one--){
            size_t layer_i = layer_i_plus_one - 1;
            if(layer_i % 2 == (SPEC::STRUCTURE_SPEC::NUM_HIDDEN_LAYERS - 1) % 2){ // we are starting with the last hidden layer where the result should go to tock
...
//        backward(network.layer_1     , input                 , d_layer_1_output, d_input);
        ...
        typename SPEC::T d_layer_input_tick[SPEC::STRUCTURE_SPEC::HIDDEN_DIM];
        typename SPEC::T d_layer_input_tock[SPEC::STRUCTURE_SPEC::HIDDEN_DIM];
        auto previous_output = SPEC::STRUCTURE_SPEC::NUM_HIDDEN_LAYERS > 0 ? network.hidden_layers[SPEC::STRUCTURE_SPEC::NUM_HIDDEN_LAYERS - 1].output : network.input_layer.output;
