...
namespace layer_in_c::utils::vector_operations{
    ...
    template <typename DEVICE, typename T, auto N>
    LAYER_IN_C_FUNCTION_PLACEMENT void add(const T v1[N], const T v2[N], T out[N]) {
        for(typename DEVICE::index_t i = 0; i < N; i++) {
            out[i] = v1[i] + v2[i];
        }
    }
    template <typename DEVICE, typename T, auto N>
    LAYER_IN_C_FUNCTION_PLACEMENT void add_accumulate(const T v1[N], const T v2[N], T out[N]) {
        for(typename DEVICE::index_t i = 0; i < N; i++) {
            out[i] += v1[i] + v2[i];
        }
    }
    template <typename DEVICE, typename T, auto N>
    LAYER_IN_C_FUNCTION_PLACEMENT void add_accumulate(T const v[N], T out[N]) {
        for(typename DEVICE::index_t i = 0; i < N; i++) {
            out[i] += v[i];
        }
    }
    ...
    template <typename DEVICE, typename T, auto N>
    LAYER_IN_C_FUNCTION_PLACEMENT void sub(T const v1[N], const T v2[N], T out[N]) {
        for(typename DEVICE::index_t i = 0; i < N; i++) {
            out[i] = v1[i] - v2[i];
        }
    }
    template <typename DEVICE, typename T, auto N>
    LAYER_IN_C_FUNCTION_PLACEMENT void sub(T const v1[N], const T v2, T out[N]) {
        for(typename DEVICE::index_t i = 0; i < N; i++) {
            out[i] = v1[i] - v2;
        }
    }
    template <typename DEVICE, typename T, auto N>
    LAYER_IN_C_FUNCTION_PLACEMENT void sub_accumulate(const T v1[N], const T v2[N], T out[N]) {
        for(typename DEVICE::index_t i = 0; i < N; i++) {
            out[i] += v1[i] - v2[i];
        }
    }
    template <typename DEVICE, typename T, auto N>
    LAYER_IN_C_FUNCTION_PLACEMENT void sub_accumulate(const T v[N], T out[N]) {
        for(typename DEVICE::index_t i = 0; i < N; i++) {
            out[i] -= v[i];
        }
    }
    ...
    template <typename DEVICE, typename T, auto N>
    LAYER_IN_C_FUNCTION_PLACEMENT T norm(const T source[N]) {
        T acc = 0;
        for(typename DEVICE::index_t i = 0; i < N; i++) {
    ...
    template <typename DEVICE, typename T, auto N>
    LAYER_IN_C_FUNCTION_PLACEMENT T mean(const T source[N]) {
        T acc = 0;
        for(typename DEVICE::index_t i = 0; i < N; i++) {
            acc += source[i];
        }
