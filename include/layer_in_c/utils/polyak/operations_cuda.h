#ifndef LAYER_IN_C_NN_UTILS_POLYAK_OPERATIONS_CUDA_H
#define LAYER_IN_C_NN_UTILS_POLYAK_OPERATIONS_CUDA_H
...
namespace layer_in_c::utils::polyak {
    ...
    void update(devices::CUDA<DEV_SPEC>& dev, Matrix<TARGET_SPEC>& target, const Matrix<SOURCE_SPEC>& source, const typename TARGET_SPEC::T polyak) {
        ...
        constexpr typename devices::CUDA<DEV_SPEC>::index_t N_BLOCKS_ROWS = LAYER_IN_C_DEVICES_CUDA_CEIL(SPEC::ROWS, BLOCKSIZE_ROWS);
        constexpr typename devices::CUDA<DEV_SPEC>::index_t N_BLOCKS_COLS = LAYER_IN_C_DEVICES_CUDA_CEIL(SPEC::COLS, BLOCKSIZE_COLS);
