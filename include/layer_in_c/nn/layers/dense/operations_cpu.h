...
namespace layer_in_c{
    ...
    void init_kaiming(devices::CPU<DEV_SPEC>& device, nn::layers::dense::Layer<LS>& layer, RNG& rng) {
        ...
        for(TI i = 0; i < LS::OUTPUT_DIM; i++) {
            layer.biases.data[index(layer.biases, 0, i)] = random::uniform_real_distribution(typename DEV_SPEC::RANDOM(), -bias_bound, bias_bound, rng);
            for(TI j = 0; j < LS::INPUT_DIM; j++) {
                layer.weights.data[index(layer.weights, i, j)] = random::uniform_real_distribution(typename DEV_SPEC::RANDOM(), -weight_bound, weight_bound, rng);
            }
        }
    }
}
