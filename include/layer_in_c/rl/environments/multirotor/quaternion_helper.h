...
namespace layer_in_c::rl::environments::multirotor{
    template <typename T>
    FUNCTION_PLACEMENT void quaternion_derivative(const T q[4], const T omega[3], T q_dot[4]) {
        // FLOPS: 3 (MAC) * 4 + 4 = 16
        ...
        utils::vector_operations::scalar_multiply<T, 4>(q_dot, 0.5);
    }


    template <typename T>
    FUNCTION_PLACEMENT void rotate_vector_by_quaternion(const T q[4], const T v[3], T v_out[3]) {
...
//    v_out[2] = q[0]*(q[1]*v[1] + q[0]*v[2] - q[2]*v[0]) + q[1]*(q[0]*v[1] + q[3]*v[0] - q[1]*v[2]) - q[2]*(q[0]*v[0] + q[2]*v[2] - q[3]*v[1]) - q[3]*(-q[1]*v[0] - q[2]*v[1] - q[3]*v[2]);
        ...
        add_accumulate<T, 3>(v, v_out); // 3 flops
    }
}
