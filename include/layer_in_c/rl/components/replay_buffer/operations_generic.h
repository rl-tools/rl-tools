...
namespace layer_in_c {
    ...
    void add(rl::components::ReplayBuffer<DEVICE, rl::components::replay_buffer::Specification<DEVICE, T, OBSERVATION_DIM, ACTION_DIM, CAPACITY>>& buffer, const T observation[OBSERVATION_DIM], const T action[ACTION_DIM], const T reward, const T next_observation[OBSERVATION_DIM], const bool terminated, const bool truncated) {
        // todo: change to memcpy?
        for(typename DEVICE::index_t i = 0; i < OBSERVATION_DIM; i++) {
