...
//                std::cout << "        diff_target_adam_per_weight: " << diff_target_adam_per_weight << std::endl;
        ...
        if(step_group.exist("critic1_target")){
            ...
            if (step_i == 0){
                ActorCriticType::CRITIC_TARGET_NETWORK_TYPE pre_critic_1_target_step;
                ...
                if (step_i >= ActorCriticType::SPEC::PARAMETERS::CRITIC_BATCH_SIZE){
                    ...
                    ActorCriticType::CRITIC_TARGET_NETWORK_TYPE post_critic_1_target;
                    lic::load(post_critic_1_target, step_group.getGroup("critic1_target"));

                    lic::update_targets(actor_critic);

                    DTYPE pre_post_diff_per_weight = abs_diff(pre_critic_1_target, post_critic_1_target)/ActorCriticType::CRITIC_NETWORK_STRUCTURE_SPEC::NUM_WEIGHTS;
                    DTYPE diff_target_per_weight = abs_diff(post_critic_1_target, actor_critic.critic_target_1)/ActorCriticType::CRITIC_NETWORK_STRUCTURE_SPEC::NUM_WEIGHTS;
                    DTYPE diff_ratio = pre_post_diff_per_weight/diff_target_per_weight;
