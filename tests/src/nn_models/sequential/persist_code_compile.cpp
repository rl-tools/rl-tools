...
TEST(BACKPROP_TOOLS_NN_MODELS_SEQUENTIAL_PERSIST_CODE_COMPILE, COMPILE) {
    ...
    bpt::evaluate(device, rl_tools_export::model::model, rl_tools_export::input::container, output, buffer);

    auto abs_diff = bpt::abs_diff(device, output, rl_tools_export::output::container);

    std::cout << "Oiriginal output:" << std::endl;
    bpt::print(device, rl_tools_export::output::container);
