...
int main(int argc, char** argv){
    ...
    if(tuple_input.num_tensors == 0){ printf("ERROR: no inputs\n"); return 1; }
    printf("Loaded %lu input tensor(s)\n", (unsigned long)tuple_input.num_tensors);

    rlt::dyn::Tensor<rlt::dyn::TensorSpecification<TI>> expected;
    if(!rlt::load(device, expected, outputs_group, "0")){ printf("ERROR: failed to load expected output\n"); return 1; }
    ...
    if(!rlt::evaluate(device, model, tuple_input, output, buffer)){ printf("ERROR: evaluate failed\n"); return 1; }
