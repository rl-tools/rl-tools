...
TEST(RL_TOOLS_RL_ENVIRONMENTS_MUJOCO_ANT, THROUGHPUT_MULTI_CORE_INDEPENDENT){
    ...
    constexpr TI NUM_STEPS_PER_THREAD = 10;
#else
...
TEST(RL_TOOLS_RL_ENVIRONMENTS_MUJOCO_ANT, THROUGHPUT_MULTI_CORE_LOCKSTEP){
    ...
    for(TI env_i = 0; env_i < NUM_THREADS; env_i++){
        threads[env_i] = std::thread([&device, &rngs, &actions, &envs, &barrier, &order, &order_mutex, env_i](){
            STATE state, next_state;
            auto rng = rngs[env_i];
            auto& env = envs[env_i];
            auto action = rlt::view(device, actions, rlt::matrix::ViewSpec<1, envp::ENVIRONMENT::ACTION_DIM>(), env_i, 0);
...
TEST(RL_TOOLS_RL_ENVIRONMENTS_MUJOCO_ANT, THROUGHPUT_MULTI_CORE_SPAWNING){
#ifdef RL_TOOLS_TESTS_CODE_COVERAGE
    constexpr TI NUM_ROLLOUT_STEPS = 2;
#else
    ...
    constexpr TI NUM_THREADS = 16;
    using ACTOR_SPEC = rlt::nn_models::mlp::Specification<T, TI, envp::ENVIRONMENT::OBSERVATION_DIM, envp::ENVIRONMENT::ACTION_DIM, 3, 256, rlt::nn::activation_functions::ActivationFunction::RELU, rlt::nn::activation_functions::IDENTITY>;
...
#endif
    constexpr TI NUM_THREADS = 16;
    using ACTOR_SPEC = rlt::nn_models::mlp::Specification<T, TI, envp::ENVIRONMENT::OBSERVATION_DIM, envp::ENVIRONMENT::ACTION_DIM, 3, 256, rlt::nn::activation_functions::ActivationFunction::TANH, rlt::nn::activation_functions::IDENTITY>;
    using CAPABILITY_ADAM = rlt::nn::layer_capability::Gradient<rlt::nn::parameters::Adam>;
    using ACTOR_TYPE = rlt::nn_models::mlp_unconditional_stddev::NeuralNetwork<CAPABILITY_ADAM, ACTOR_SPEC>;
...
#ifdef RL_TOOLS_TESTS_CODE_COVERAGE
    constexpr TI NUM_STEPS_PER_ENVIRONMENT = 10;
#else
...
#ifdef RL_TOOLS_TESTS_CODE_COVERAGE
    constexpr TI NUM_FULL_STEPS = 2;
    constexpr TI NUM_STEPS_PER_ENVIRONMENT = 2;
#else
