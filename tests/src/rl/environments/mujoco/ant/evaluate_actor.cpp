...
TEST(LAYER_IN_C_RL_ENVIRONMENTS_MUJOCO_ANT, EVALUATE_ACTOR){
    ...
    for (const auto& file : std::filesystem::directory_iterator(actor_checkpoints_dir)) {
        if (file.is_regular_file()) {
            actor_checkpoints.push_back(file.path());
        }
    }
