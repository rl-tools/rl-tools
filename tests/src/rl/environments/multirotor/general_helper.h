...
template <typename T, int N>
FUNCTION_PLACEMENT void vector_add(const T v1[N], const T v2[N], T out[N]) {
    for(int i = 0; i < N; i++) {
        out[i] = v1[i] + v2[i];
    }
}
template <typename T, int N>
FUNCTION_PLACEMENT void vector_add_accumulate(const T v1[N], const T v2[N], T out[N]) {
    for(int i = 0; i < N; i++) {
        out[i] += v1[i] + v2[i];
    }
}
template <typename T, int N>
FUNCTION_PLACEMENT void vector_add_accumulate(T const v[N], T out[N]) {
    for(int i = 0; i < N; i++) {
        out[i] += v[i];
    }
}
...
template <typename T, int N>
FUNCTION_PLACEMENT void vector_sub(T const v1[N], const T v2[N], T out[N]) {
    for(int i = 0; i < N; i++) {
        out[i] = v1[i] - v2[i];
    }
}
template <typename T, int N>
FUNCTION_PLACEMENT void vector_sub_accumulate(const T v1[N], const T v2[N], T out[N]) {
    for(int i = 0; i < N; i++) {
        out[i] += v1[i] - v2[i];
    }
}
template <typename T, int N>
FUNCTION_PLACEMENT void vector_sub_accumulate(const T v[N], T out[N]) {
    for(int i = 0; i < N; i++) {
        out[i] -= v[i];
    }
}
