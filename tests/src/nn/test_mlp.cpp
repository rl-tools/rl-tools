...
protected:
    NeuralNetworkTestLoadWeights(){
        ...
        lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, INPUT_DIM, lic::matrix::layouts::RowMajorAlignment<NN_DEVICE::index_t>>> input_matrix;
        input_matrix._data = input;
        lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, OUTPUT_DIM, lic::matrix::layouts::RowMajorAlignment<NN_DEVICE::index_t>>> output_matrix;
...
//        lic::forward(device, network, input);
        ...
        lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, OUTPUT_DIM, lic::matrix::layouts::RowMajorAlignment<NN_DEVICE::index_t>>> d_loss_d_output_matrix;
...
//        lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, OUTPUT_DIM>> d_loss_d_output_matrix = {d_loss_d_output};
        lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, INPUT_DIM, lic::matrix::layouts::RowMajorAlignment<NN_DEVICE::index_t>>> d_input_matrix;
...
TEST_F(LAYER_IN_C_NN_MLP_ADAM_UPDATE, AdamUpdate) {
    ...
    lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, INPUT_DIM, lic::matrix::layouts::RowMajorAlignment<typename DEVICE::index_t>>> input_matrix;
    input_matrix._data = input;
    lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, OUTPUT_DIM, lic::matrix::layouts::RowMajorAlignment<typename DEVICE::index_t>>> output_matrix;
    output_matrix._data = output;
    lic::forward(device, network, input_matrix);
    DTYPE d_loss_d_output[OUTPUT_DIM];
    lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, OUTPUT_DIM, lic::matrix::layouts::RowMajorAlignment<typename DEVICE::index_t>>> d_loss_d_output_matrix;
    d_loss_d_output_matrix._data = d_loss_d_output;
    lic::nn::loss_functions::d_mse_d_x(device, network.output_layer.output, output_matrix, d_loss_d_output_matrix);
    DTYPE d_input[INPUT_DIM];
    lic::zero_gradient(device, network);
...
TEST_F(LAYER_IN_C_NN_MLP_OVERFIT_BATCH, OverfitBatch) {
    ...
    for (int batch_i=0; batch_i < n_iter; batch_i++){
        ...
        for (int sample_i=0; sample_i < batch_size; sample_i++){
            ...
            lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, INPUT_DIM, lic::matrix::layouts::RowMajorAlignment<typename NN_DEVICE::index_t>>> input_matrix;
            input_matrix._data = input;
            lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, OUTPUT_DIM>> output_matrix;
            output_matrix._data = output;
            lic::forward(device, network, input_matrix);
            DTYPE d_loss_d_output[OUTPUT_DIM];
            lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, OUTPUT_DIM>> d_loss_d_output_matrix;
            d_loss_d_output_matrix._data = d_loss_d_output;
            ...
            DTYPE d_input[INPUT_DIM];
            lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, INPUT_DIM>> d_input_matrix;
            d_input_matrix._data = d_input;
            lic::backward(device, network, input_matrix, d_loss_d_output_matrix, d_input_matrix, network_buffers);
        }
        loss /= batch_size;
    ...
    for(int batch_i_real=0; batch_i_real < n_batches; batch_i_real++){
        ...
        for (int batch_i=0; batch_i < n_iter; batch_i++){
            ...
            for (int sample_i=0; sample_i < batch_size; sample_i++){
                ...
                standardise<DTYPE,  INPUT_DIM>(X_train[batch_i_real * batch_size + sample_i].data(), X_mean.data(), X_std.data(), input);
                standardise<DTYPE, OUTPUT_DIM>(Y_train[batch_i_real * batch_size + sample_i].data(), Y_mean.data(), Y_std.data(), output);
                lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, INPUT_DIM>> input_matrix;
                input_matrix._data = input;
                lic::Matrix<lic::matrix::Specification<DTYPE, NN_DEVICE::index_t, 1, OUTPUT_DIM>> output_matrix;
                output_matrix._data = output;
                lic::forward(device, network, input_matrix);
