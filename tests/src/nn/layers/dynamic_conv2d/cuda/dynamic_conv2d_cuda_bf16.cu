...
// ======================== Full (non-depthwise) convolution BF16 tests ========================
static constexpr float FULL_FWD_EPSILON = 0.05;
static constexpr float FULL_BWD_D_DATA_EPSILON = 0.3;
static constexpr float FULL_BWD_D_KW_EPSILON = 0.2;

template <typename CONV_CONFIG_CPU, typename CONV_CONFIG_CUDA, TI BATCH_SIZE, TI HEIGHT, TI WIDTH, TI CHANNELS_IN>
void test_dynamic_full_conv2d_cuda_bf16(const char* label){
    DEVICE_CPU device_cpu;
    DEVICE_CUDA device_cuda;
    rlt::init(device_cuda);
    RNG_CPU rng_cpu;
    ...
    constexpr TI CHANNELS_OUT = CONV_CONFIG_CPU::OUTPUT_CHANNELS;
    constexpr float LOCAL_FWD_EPS = FULL_FWD_EPSILON;
    constexpr float LOCAL_BWD_DATA_EPS = FULL_BWD_D_DATA_EPSILON;
    constexpr float LOCAL_BWD_KW_EPS = FULL_BWD_D_KW_EPSILON;
    using INPUT_SHAPE_CPU = rlt::tensor::Shape<TI, BATCH_SIZE, HEIGHT, WIDTH, CHANNELS_IN>;
    using INPUT_SHAPE_CUDA = rlt::tensor::Shape<TI_CUDA, BATCH_SIZE, HEIGHT, WIDTH, CHANNELS_IN>;
    using CAPABILITY = rlt::nn::capability::Gradient<rlt::nn::parameters::Gradient>;
    using LAYER_CPU = rlt::nn::layers::dynamic_conv2d::Layer<CONV_CONFIG_CPU, CAPABILITY, INPUT_SHAPE_CPU>;
    using LAYER_CUDA = rlt::nn::layers::dynamic_conv2d::Layer<CONV_CONFIG_CUDA, CAPABILITY, INPUT_SHAPE_CUDA>;
    ...
    using OUTPUT_SHAPE_CPU = rlt::tensor::Shape<TI, BATCH_SIZE, OH, OW, CHANNELS_OUT>;
    using OUTPUT_SHAPE_CUDA = rlt::tensor::Shape<TI_CUDA, BATCH_SIZE, OH, OW, CHANNELS_OUT>;
    using KW_SHAPE_CPU = rlt::tensor::Shape<TI, BATCH_SIZE, CHANNELS_OUT, CHANNELS_IN, KH, KW>;
    using KW_SHAPE_CUDA = rlt::tensor::Shape<TI_CUDA, BATCH_SIZE, CHANNELS_OUT, CHANNELS_IN, KH, KW>;
    ...
    T fwd_diff = rlt::abs_diff(device_cpu, layer_cpu.output, output_cuda_host) / decltype(output_cuda_host)::SPEC::SIZE;
    std::cout << label << " BF16 FORWARD FP32(CPU) vs BF16(CUDA): " << fwd_diff << std::endl;
    EXPECT_LT(fwd_diff, LOCAL_FWD_EPS);
    ...
    T d_data_diff = rlt::abs_diff(device_cpu, d_data_cpu, d_data_cuda_host) / decltype(d_data_cpu)::SPEC::SIZE;
    std::cout << label << " BF16 BACKWARD d_data: " << d_data_diff << std::endl;
    EXPECT_LT(d_data_diff, LOCAL_BWD_DATA_EPS);

    T d_kw_diff = rlt::abs_diff(device_cpu, d_kw_cpu, d_kw_cuda_host) / decltype(d_kw_cpu)::SPEC::SIZE;
    std::cout << label << " BF16 BACKWARD d_kw: " << d_kw_diff << std::endl;
    EXPECT_LT(d_kw_diff, LOCAL_BWD_KW_EPS);
...
TEST(NN_LAYERS_DYNAMIC_CONV2D_CUDA_BF16, FULL_BASIC_3X3){
    using CONFIG_CPU = rlt::nn::layers::dynamic_conv2d::Configuration<TYPE_POLICY, TI, 3, 3, 1, 1, 0, 0, rlt::nn::activation_functions::ActivationFunction::IDENTITY, 16>;
    using CONFIG_CUDA = rlt::nn::layers::dynamic_conv2d::Configuration<TYPE_POLICY_BF16, TI_CUDA, 3, 3, 1, 1, 0, 0, rlt::nn::activation_functions::ActivationFunction::IDENTITY, 16>;
    test_dynamic_full_conv2d_cuda_bf16<CONFIG_CPU, CONFIG_CUDA, 2, 8, 8, 8>("FULL_BASIC_3X3");
}
TEST(NN_LAYERS_DYNAMIC_CONV2D_CUDA_BF16, FULL_PADDED_3X3){
    using CONFIG_CPU = rlt::nn::layers::dynamic_conv2d::Configuration<TYPE_POLICY, TI, 3, 3, 1, 1, 1, 1, rlt::nn::activation_functions::ActivationFunction::IDENTITY, 4>;
    using CONFIG_CUDA = rlt::nn::layers::dynamic_conv2d::Configuration<TYPE_POLICY_BF16, TI_CUDA, 3, 3, 1, 1, 1, 1, rlt::nn::activation_functions::ActivationFunction::IDENTITY, 4>;
    test_dynamic_full_conv2d_cuda_bf16<CONFIG_CPU, CONFIG_CUDA, 2, 8, 8, 8>("FULL_PADDED_3X3");
}
TEST(NN_LAYERS_DYNAMIC_CONV2D_CUDA_BF16, FULL_STRIDED_3X3){
    using CONFIG_CPU = rlt::nn::layers::dynamic_conv2d::Configuration<TYPE_POLICY, TI, 3, 3, 2, 2, 1, 1, rlt::nn::activation_functions::ActivationFunction::IDENTITY, 32>;
    using CONFIG_CUDA = rlt::nn::layers::dynamic_conv2d::Configuration<TYPE_POLICY_BF16, TI_CUDA, 3, 3, 2, 2, 1, 1, rlt::nn::activation_functions::ActivationFunction::IDENTITY, 32>;
    test_dynamic_full_conv2d_cuda_bf16<CONFIG_CPU, CONFIG_CUDA, 2, 16, 16, 16>("FULL_STRIDED_3X3");
}
TEST(NN_LAYERS_DYNAMIC_CONV2D_CUDA_BF16, FULL_RELU_3X3){
    using CONFIG_CPU = rlt::nn::layers::dynamic_conv2d::Configuration<TYPE_POLICY, TI, 3, 3, 1, 1, 1, 1, rlt::nn::activation_functions::ActivationFunction::RELU, 16>;
    using CONFIG_CUDA = rlt::nn::layers::dynamic_conv2d::Configuration<TYPE_POLICY_BF16, TI_CUDA, 3, 3, 1, 1, 1, 1, rlt::nn::activation_functions::ActivationFunction::RELU, 16>;
    test_dynamic_full_conv2d_cuda_bf16<CONFIG_CPU, CONFIG_CUDA, 2, 8, 8, 8>("FULL_RELU_3X3");
}
TEST(NN_LAYERS_DYNAMIC_CONV2D_CUDA_BF16, FULL_STRIDED_RELU){
    using CONFIG_CPU = rlt::nn::layers::dynamic_conv2d::Configuration<TYPE_POLICY, TI, 3, 3, 2, 2, 1, 1, rlt::nn::activation_functions::ActivationFunction::RELU, 8>;
    using CONFIG_CUDA = rlt::nn::layers::dynamic_conv2d::Configuration<TYPE_POLICY_BF16, TI_CUDA, 3, 3, 2, 2, 1, 1, rlt::nn::activation_functions::ActivationFunction::RELU, 8>;
    test_dynamic_full_conv2d_cuda_bf16<CONFIG_CPU, CONFIG_CUDA, 2, 16, 16, 16>("FULL_STRIDED_RELU");
}
