// Per-layer CUDA test for avg_pool2d (from ResNet-18)
...
using AVGPOOL_CONFIG = rlt::nn_models::resnet18::AVGPOOL_CONFIG<TYPE_POLICY, TI>;
using AVGPOOL_CONFIG_CUDA = rlt::nn_models::resnet18::AVGPOOL_CONFIG<TYPE_POLICY, TI_CUDA>;
using INPUT_SHAPE = rlt::tensor::Shape<TI, 1, 7, 7, 512>;
using INPUT_SHAPE_CUDA = rlt::tensor::Shape<TI_CUDA, 1, 7, 7, 512>;
...
using AP_FWD_CPU = rlt::nn::layers::avg_pool2d::Layer<AVGPOOL_CONFIG, FWD_CAPABILITY, INPUT_SHAPE>;
using AP_FWD_CUDA = rlt::nn::layers::avg_pool2d::Layer<AVGPOOL_CONFIG_CUDA, FWD_CAPABILITY, INPUT_SHAPE_CUDA>;
using AP_GRAD_CPU = rlt::nn::layers::avg_pool2d::Layer<AVGPOOL_CONFIG, GRAD_CAPABILITY, INPUT_SHAPE>;
using AP_GRAD_CUDA = rlt::nn::layers::avg_pool2d::Layer<AVGPOOL_CONFIG_CUDA, GRAD_CAPABILITY, INPUT_SHAPE_CUDA>;

TEST(NN_LAYERS_AVG_POOL2D_CUDA, EVALUATE) {
    ...
    rlt::load(device_cpu, ic, tg, "after_layer4_block1");
    ...
    using OC = typename AP_FWD_CPU::OUTPUT_SHAPE;
    using OCU = typename AP_FWD_CUDA::OUTPUT_SHAPE;
...
TEST(NN_LAYERS_AVG_POOL2D_CUDA, FORWARD) {
    DEVICE_CPU device_cpu; DEVICE_CUDA device_cuda; rlt::init(device_cuda);
    RNG_CPU rng_cpu; rlt::malloc(device_cpu, rng_cpu); rlt::init(device_cpu, rng_cpu, 0);
    const char *dp = RL_TOOLS_MACRO_TO_STR(RL_TOOLS_TEST_DATA_PATH);
    auto file = HighFive::File(std::string(dp) + "/resnet18_test_data.h5", HighFive::File::ReadOnly);
    ...
    rlt::malloc(device_cuda, lcu); rlt::malloc(device_cuda, bcu);
    rlt::Tensor<rlt::tensor::Specification<T, TI, INPUT_SHAPE>> ic; rlt::malloc(device_cpu, ic);
    auto tg2 = rlt::get_group(device_cpu, file, "test_data");
    rlt::load(device_cpu, ic, tg2, "after_layer4_block1");
    rlt::Tensor<rlt::tensor::Specification<T, TI_CUDA, INPUT_SHAPE_CUDA>> icu;
    rlt::malloc(device_cuda, icu); rlt::copy(device_cpu, device_cuda, ic, icu);
...
TEST(NN_LAYERS_AVG_POOL2D_CUDA, BACKWARD) {
    DEVICE_CPU device_cpu; DEVICE_CUDA device_cuda; rlt::init(device_cuda);
    RNG_CPU rng_cpu; rlt::malloc(device_cpu, rng_cpu); rlt::init(device_cpu, rng_cpu, 0);
    const char *dp = RL_TOOLS_MACRO_TO_STR(RL_TOOLS_TEST_DATA_PATH);
    auto file = HighFive::File(std::string(dp) + "/resnet18_test_data.h5", HighFive::File::ReadOnly);
    ...
    rlt::malloc(device_cuda, lcu); rlt::malloc(device_cuda, bcu);
    rlt::Tensor<rlt::tensor::Specification<T, TI, INPUT_SHAPE>> ic; rlt::malloc(device_cpu, ic);
    auto tg3 = rlt::get_group(device_cpu, file, "test_data");
    rlt::load(device_cpu, ic, tg3, "after_layer4_block1");
    rlt::Tensor<rlt::tensor::Specification<T, TI_CUDA, INPUT_SHAPE_CUDA>> icu;
    rlt::malloc(device_cuda, icu); rlt::copy(device_cpu, device_cuda, ic, icu);
    rlt::Mode<rlt::mode::Evaluation<>> eval_mode;
    rlt::forward(device_cpu, lc, ic, bc, rng_cpu, eval_mode);
    ...
    rlt::forward(device_cuda, lcu, icu, bcu, rng_cuda, eval_mode);
    using OC = typename AP_GRAD_CPU::OUTPUT_SHAPE;
    using OCU = typename AP_GRAD_CUDA::OUTPUT_SHAPE;
    ...
    std::cout << "AvgPool2d BACKWARD d_input: " << di << std::endl;
