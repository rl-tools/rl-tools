...
TEST(NN_LAYERS_CONV2D_CUDA, BACKWARD) {
    ...
    rlt::forward(device_cuda, lcu, icu, bcu, rng_cuda, eval_mode);
    rlt::backward_full(device_cuda, lcu, icu, docu, dicu, bcu, eval_mode);
    STEM_GRAD_CPU accumulated; rlt::malloc(device_cpu, accumulated);
    rlt::copy(device_cuda, device_cpu, lcu, accumulated);
    rlt::scale(device_cpu, accumulated.weights.gradient, (T)0.5);
    rlt::scale(device_cpu, accumulated.biases.gradient, (T)0.5);
    rlt::scale(device_cpu, accumulated.norm.gamma.gradient, (T)0.5);
    rlt::scale(device_cpu, accumulated.norm.beta.gradient, (T)0.5);
    EXPECT_LT(rlt::abs_diff(device_cpu, lch.weights.gradient, accumulated.weights.gradient) / ((T)1 + rlt::abs_sum(device_cpu, lch.weights.gradient)), FWD_EPSILON);
    EXPECT_LT(rlt::abs_diff(device_cpu, lch.biases.gradient, accumulated.biases.gradient) / ((T)1 + rlt::abs_sum(device_cpu, lch.biases.gradient)), FWD_EPSILON);
    EXPECT_LT(rlt::abs_diff(device_cpu, lch.norm.gamma.gradient, accumulated.norm.gamma.gradient) / ((T)1 + rlt::abs_sum(device_cpu, lch.norm.gamma.gradient)), FWD_EPSILON);
    EXPECT_LT(rlt::abs_diff(device_cpu, lch.norm.beta.gradient, accumulated.norm.beta.gradient) / ((T)1 + rlt::abs_sum(device_cpu, lch.norm.beta.gradient)), FWD_EPSILON);
    rlt::free(device_cpu, accumulated);
    rlt::free(device_cpu, lc); rlt::free(device_cpu, bc);
    rlt::free(device_cuda, lcu); rlt::free(device_cuda, bcu);
    rlt::free(device_cpu, ic); rlt::free(device_cuda, icu);
