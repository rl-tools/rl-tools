// BF16 CUDA test for conv2d (BN+ReLU stem from ResNet-18)
...
static constexpr float FWD_EPSILON = 0.006;      // observed: 0.0045
static constexpr float BWD_DI_EPSILON = 0.07;    // observed: 0.051
static constexpr float BWD_DW_EPSILON = 2.0;     // observed: 1.58
static constexpr float BWD_DG_EPSILON = 18.0;    // observed: 15.20
static constexpr float BWD_DB_EPSILON = 13.0;    // observed: 10.73
using STEM_CONFIG = rlt::nn_models::resnet18::STEM_CONV_CONFIG<TYPE_POLICY, TI>;
using STEM_CONFIG_CUDA = rlt::nn_models::resnet18::STEM_CONV_CONFIG<TYPE_POLICY_BF16, TI_CUDA>;
...
TEST(NN_LAYERS_CONV2D_CUDA_BF16, EVALUATE) {
    ...
    std::cout << "Conv2d BF16 EVALUATE FP32(CPU) vs BF16(CUDA): " << diff << std::endl;
...
TEST(NN_LAYERS_CONV2D_CUDA_BF16, FORWARD) {
    DEVICE_CPU device_cpu; DEVICE_CUDA device_cuda; rlt::init(device_cuda);
    RNG_CPU rng_cpu; rlt::malloc(device_cpu, rng_cpu); rlt::init(device_cpu, rng_cpu, 0);
    const char *dp = RL_TOOLS_MACRO_TO_STR(RL_TOOLS_TEST_DATA_PATH);
    auto file = HighFive::File(std::string(dp) + "/resnet18_test_data.h5", HighFive::File::ReadOnly);
    ...
    std::cout << "Conv2d BF16 FORWARD FP32(CPU) vs BF16(CUDA): " << diff << std::endl;
    EXPECT_LT(diff, FWD_EPSILON);
    rlt::free(device_cpu, lc); rlt::free(device_cpu, bc);
    rlt::free(device_cuda, lcu); rlt::free(device_cuda, bcu);
    rlt::free(device_cpu, ic); rlt::free(device_cuda, icu);
...
TEST(NN_LAYERS_CONV2D_CUDA_BF16, BACKWARD) {
    DEVICE_CPU device_cpu; DEVICE_CUDA device_cuda; rlt::init(device_cuda);
    RNG_CPU rng_cpu; rlt::malloc(device_cpu, rng_cpu); rlt::init(device_cpu, rng_cpu, 0);
    const char *dp = RL_TOOLS_MACRO_TO_STR(RL_TOOLS_TEST_DATA_PATH);
    auto file = HighFive::File(std::string(dp) + "/resnet18_test_data.h5", HighFive::File::ReadOnly);
    ...
    std::cout << "Conv2d BF16 BACKWARD d_input: " << di << std::endl; EXPECT_LT(di, BWD_DI_EPSILON);
    ...
    std::cout << "Conv2d BF16 BACKWARD d_weights: " << dw << std::endl; EXPECT_LT(dw, BWD_DW_EPSILON);
    T dg = rlt::abs_diff(device_cpu, lc.norm.gamma.gradient, lch.norm.gamma.gradient) / decltype(lc.norm.gamma.gradient)::SPEC::SIZE;
    std::cout << "Conv2d BF16 BACKWARD d_gamma: " << dg << std::endl; EXPECT_LT(dg, BWD_DG_EPSILON);
    T db = rlt::abs_diff(device_cpu, lc.norm.beta.gradient, lch.norm.beta.gradient) / decltype(lc.norm.beta.gradient)::SPEC::SIZE;
    std::cout << "Conv2d BF16 BACKWARD d_beta: " << db << std::endl; EXPECT_LT(db, BWD_DB_EPSILON);
    rlt::free(device_cpu, lc); rlt::free(device_cpu, bc);
    rlt::free(device_cuda, lcu); rlt::free(device_cuda, bcu);
    rlt::free(device_cpu, ic); rlt::free(device_cuda, icu);
