...
static constexpr T NORM_EPSILON = 1e-7;  // Slightly relaxed for normalized layers

// ======================== Plain Conv2d test helper ========================
...
// ======================== Normalized Conv2d test helper ========================
template <typename CONV_CONFIG, TI BATCH_SIZE, TI HEIGHT, TI WIDTH, TI INPUT_CHANNELS>
void test_conv2d_norm_case(const std::string& test_case_name, T tolerance = NORM_EPSILON) {
    DEVICE device;
    DEVICE::SPEC::RANDOM::ENGINE<> rng;
    rlt::malloc(device, rng);
    rlt::init(device, rng, 0);
    ...
    rlt::Tensor<rlt::tensor::Specification<T, TI, OUTPUT_SHAPE>> expected_output, d_output;
    rlt::Tensor<rlt::tensor::Specification<T, TI, WEIGHTS_SHAPE>> d_weights_expected;
    rlt::Tensor<rlt::tensor::Specification<T, TI, BIASES_SHAPE>> d_biases_expected, d_gamma_expected, d_beta_expected;
    ...
    rlt::malloc(device, d_weights_expected);
    rlt::malloc(device, d_biases_expected);
    rlt::malloc(device, d_gamma_expected);
    rlt::malloc(device, d_beta_expected);
    ...
    rlt::load(device, layer.norm.gamma.parameters, group, "gamma");
    rlt::load(device, layer.norm.beta.parameters, group, "beta");
    rlt::load(device, d_gamma_expected, group, "d_gamma");
    rlt::load(device, d_beta_expected, group, "d_beta");
    ...
    T forward_diff = rlt::abs_diff(device, expected_output, layer.output) / decltype(expected_output)::SPEC::SIZE;
    std::cout << "  Forward abs_diff (per element): " << forward_diff << std::endl;
    ASSERT_LT(forward_diff, tolerance) << "Forward pass mismatch for " << test_case_name;
    ...
    T d_input_diff = rlt::abs_diff(device, d_input, d_input_expected) / decltype(d_input)::SPEC::SIZE;
    std::cout << "  d_input abs_diff (per element): " << d_input_diff << std::endl;
    ASSERT_LT(d_input_diff, tolerance) << "d_input mismatch for " << test_case_name;

    // Check d_weights
    T d_weights_diff = rlt::abs_diff(device, layer.weights.gradient, d_weights_expected) / decltype(d_weights_expected)::SPEC::SIZE;
    std::cout << "  d_weights abs_diff (per element): " << d_weights_diff << std::endl;
    ASSERT_LT(d_weights_diff, tolerance) << "d_weights mismatch for " << test_case_name;

    // Check d_biases
    T d_biases_diff = rlt::abs_diff(device, layer.biases.gradient, d_biases_expected) / decltype(d_biases_expected)::SPEC::SIZE;
    std::cout << "  d_biases abs_diff (per element): " << d_biases_diff << std::endl;
    ASSERT_LT(d_biases_diff, tolerance) << "d_biases mismatch for " << test_case_name;

    // Check d_gamma
    T d_gamma_diff = rlt::abs_diff(device, layer.norm.gamma.gradient, d_gamma_expected) / decltype(d_gamma_expected)::SPEC::SIZE;
    std::cout << "  d_gamma abs_diff (per element): " << d_gamma_diff << std::endl;
    ASSERT_LT(d_gamma_diff, tolerance) << "d_gamma mismatch for " << test_case_name;

    // Check d_beta
    T d_beta_diff = rlt::abs_diff(device, layer.norm.beta.gradient, d_beta_expected) / decltype(d_beta_expected)::SPEC::SIZE;
    std::cout << "  d_beta abs_diff (per element): " << d_beta_diff << std::endl;
    ASSERT_LT(d_beta_diff, tolerance) << "d_beta mismatch for " << test_case_name;
    ...
    rlt::free(device, d_weights_expected);
    rlt::free(device, d_biases_expected);
    rlt::free(device, d_gamma_expected);
    rlt::free(device, d_beta_expected);
}
...
// ======================== BatchNorm Conv2d Tests ========================
TEST(RL_TOOLS_NN_LAYERS_CONV2D, BATCHNORM_3X3) {
    using CONFIG = rlt::nn::layers::conv2d::Configuration<TYPE_POLICY, TI, 16, 3, 3, 1, 1, 1, 1,
        rlt::nn::activation_functions::ActivationFunction::IDENTITY,
        rlt::nn::layers::conv2d::Normalization::BATCH_NORM>;
    test_conv2d_norm_case<CONFIG, 4, 8, 8, 3>("batchnorm_3x3");
}

TEST(RL_TOOLS_NN_LAYERS_CONV2D, BATCHNORM_RELU) {
    using CONFIG = rlt::nn::layers::conv2d::Configuration<TYPE_POLICY, TI, 8, 3, 3, 1, 1, 1, 1,
        rlt::nn::activation_functions::ActivationFunction::RELU,
        rlt::nn::layers::conv2d::Normalization::BATCH_NORM>;
    test_conv2d_norm_case<CONFIG, 4, 8, 8, 3>("batchnorm_relu");
}

// ======================== LayerNorm Conv2d Tests ========================
TEST(RL_TOOLS_NN_LAYERS_CONV2D, LAYERNORM_3X3) {
    using CONFIG = rlt::nn::layers::conv2d::Configuration<TYPE_POLICY, TI, 16, 3, 3, 1, 1, 1, 1,
        rlt::nn::activation_functions::ActivationFunction::IDENTITY,
        rlt::nn::layers::conv2d::Normalization::LAYER_NORM>;
    test_conv2d_norm_case<CONFIG, 2, 8, 8, 3>("layernorm_3x3");
}

TEST(RL_TOOLS_NN_LAYERS_CONV2D, LAYERNORM_RELU) {
    using CONFIG = rlt::nn::layers::conv2d::Configuration<TYPE_POLICY, TI, 8, 3, 3, 1, 1, 1, 1,
        rlt::nn::activation_functions::ActivationFunction::RELU,
        rlt::nn::layers::conv2d::Normalization::LAYER_NORM>;
    test_conv2d_norm_case<CONFIG, 2, 8, 8, 3>("layernorm_relu");
}
