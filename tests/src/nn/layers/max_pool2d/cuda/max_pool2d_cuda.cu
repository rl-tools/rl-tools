// Per-layer CUDA test for max_pool2d (from ResNet-18)
...
using MAXPOOL_CONFIG_CUDA = rlt::nn_models::resnet18::MAXPOOL_CONFIG<TYPE_POLICY, TI_CUDA>;
using INPUT_SHAPE = rlt::tensor::Shape<TI, 1, 112, 112, 64>;
using INPUT_SHAPE_CUDA = rlt::tensor::Shape<TI_CUDA, 1, 112, 112, 64>;
...
using MP_FWD_CPU = rlt::nn::layers::max_pool2d::Layer<MAXPOOL_CONFIG, FWD_CAPABILITY, INPUT_SHAPE>;
using MP_FWD_CUDA = rlt::nn::layers::max_pool2d::Layer<MAXPOOL_CONFIG_CUDA, FWD_CAPABILITY, INPUT_SHAPE_CUDA>;
using MP_GRAD_CPU = rlt::nn::layers::max_pool2d::Layer<MAXPOOL_CONFIG, GRAD_CAPABILITY, INPUT_SHAPE>;
using MP_GRAD_CUDA = rlt::nn::layers::max_pool2d::Layer<MAXPOOL_CONFIG_CUDA, GRAD_CAPABILITY, INPUT_SHAPE_CUDA>;

TEST(NN_LAYERS_MAX_POOL2D_CUDA, EVALUATE) {
    ...
    MP_FWD_CPU lc; typename MP_FWD_CPU::template Buffer<true> bc;
    rlt::malloc(device_cpu, lc); rlt::malloc(device_cpu, bc);
    MP_FWD_CUDA lcu; typename MP_FWD_CUDA::template Buffer<true> bcu;
    ...
    rlt::load(device_cpu, ic, tg, "after_stem");
    ...
    using OC = typename MP_FWD_CPU::OUTPUT_SHAPE;
    using OCU = typename MP_FWD_CUDA::OUTPUT_SHAPE;
...
TEST(NN_LAYERS_MAX_POOL2D_CUDA, FORWARD) {
    DEVICE_CPU device_cpu; DEVICE_CUDA device_cuda; rlt::init(device_cuda);
    RNG_CPU rng_cpu; rlt::malloc(device_cpu, rng_cpu); rlt::init(device_cpu, rng_cpu, 0);
    const char *dp = RL_TOOLS_MACRO_TO_STR(RL_TOOLS_TEST_DATA_PATH);
    auto file = HighFive::File(std::string(dp) + "/resnet18_test_data.h5", HighFive::File::ReadOnly);
    MP_GRAD_CPU lc; typename MP_GRAD_CPU::template Buffer<true> bc;
    rlt::malloc(device_cpu, lc); rlt::malloc(device_cpu, bc);
    MP_GRAD_CUDA lcu; typename MP_GRAD_CUDA::template Buffer<true> bcu;
    rlt::malloc(device_cuda, lcu); rlt::malloc(device_cuda, bcu);
    rlt::Tensor<rlt::tensor::Specification<T, TI, INPUT_SHAPE>> ic; rlt::malloc(device_cpu, ic);
    auto tg_fwd = rlt::get_group(device_cpu, file, "test_data");
    rlt::load(device_cpu, ic, tg_fwd, "after_stem");
    rlt::Tensor<rlt::tensor::Specification<T, TI_CUDA, INPUT_SHAPE_CUDA>> icu;
    rlt::malloc(device_cuda, icu); rlt::copy(device_cpu, device_cuda, ic, icu);
...
TEST(NN_LAYERS_MAX_POOL2D_CUDA, BACKWARD) {
    DEVICE_CPU device_cpu; DEVICE_CUDA device_cuda; rlt::init(device_cuda);
    RNG_CPU rng_cpu; rlt::malloc(device_cpu, rng_cpu); rlt::init(device_cpu, rng_cpu, 0);
    const char *dp = RL_TOOLS_MACRO_TO_STR(RL_TOOLS_TEST_DATA_PATH);
    auto file = HighFive::File(std::string(dp) + "/resnet18_test_data.h5", HighFive::File::ReadOnly);
    ...
    rlt::malloc(device_cuda, lcu); rlt::malloc(device_cuda, bcu);
    rlt::Tensor<rlt::tensor::Specification<T, TI, INPUT_SHAPE>> ic; rlt::malloc(device_cpu, ic);
    auto tg_bwd = rlt::get_group(device_cpu, file, "test_data");
    rlt::load(device_cpu, ic, tg_bwd, "after_stem");
    rlt::Tensor<rlt::tensor::Specification<T, TI_CUDA, INPUT_SHAPE_CUDA>> icu;
    rlt::malloc(device_cuda, icu); rlt::copy(device_cpu, device_cuda, ic, icu);
    rlt::Mode<rlt::mode::Evaluation<>> eval_mode;
    rlt::forward(device_cpu, lc, ic, bc, rng_cpu, eval_mode);
    ...
    rlt::forward(device_cuda, lcu, icu, bcu, rng_cuda, eval_mode);
    using OC = typename MP_GRAD_CPU::OUTPUT_SHAPE;
    using OCU = typename MP_GRAD_CUDA::OUTPUT_SHAPE;
    ...
    std::cout << "MaxPool2d BACKWARD d_input: " << di << std::endl;
