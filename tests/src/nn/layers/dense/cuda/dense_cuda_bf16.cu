// BF16 CUDA test for dense (FC from ResNet-18)
...
// Tight per-metric tolerances (observed values in comments)
static constexpr float FWD_EPSILON = 0.015;     // observed: 0.0123
static constexpr float BWD_DI_EPSILON = 0.09;   // observed: 0.0736
static constexpr float BWD_DW_EPSILON = 0.001;  // observed: 9.2e-5
static constexpr float BWD_DB_EPSILON = 0.001;  // observed: 0
using FC_CONFIG = rlt::nn_models::resnet18::FC_CONFIG<TYPE_POLICY, TI>;
using FC_CONFIG_CUDA = rlt::nn_models::resnet18::FC_CONFIG<TYPE_POLICY_BF16, TI_CUDA>;
using INPUT_SHAPE = rlt::tensor::Shape<TI, 1, 512>;
using INPUT_SHAPE_CUDA = rlt::tensor::Shape<TI_CUDA, 1, 512>;
...
using FC_FWD_CPU = rlt::nn::layers::dense::Layer<FC_CONFIG, FWD_CAPABILITY, INPUT_SHAPE>;
using FC_FWD_CUDA = rlt::nn::layers::dense::Layer<FC_CONFIG_CUDA, FWD_CAPABILITY, INPUT_SHAPE_CUDA>;
using FC_GRAD_CPU = rlt::nn::layers::dense::Layer<FC_CONFIG, GRAD_CAPABILITY, INPUT_SHAPE>;
using FC_GRAD_CUDA = rlt::nn::layers::dense::Layer<FC_CONFIG_CUDA, GRAD_CAPABILITY, INPUT_SHAPE_CUDA>;

TEST(NN_LAYERS_DENSE_CUDA_BF16, EVALUATE) {
    ...
    auto fc_group = rlt::get_group(device_cpu, lg, "11");
    ASSERT_TRUE(rlt::load(device_cpu, lc, fc_group));
    FC_FWD_CUDA lcu; typename FC_FWD_CUDA::template Buffer<true> bcu;
    ...
    rlt::load(device_cpu, ic, tg, "after_avgpool");
    rlt::Tensor<rlt::tensor::Specification<T_BF16, TI_CUDA, INPUT_SHAPE_CUDA>> icu;
    rlt::malloc(device_cuda, icu); rlt::copy(device_cpu, device_cuda, ic, icu);
    using OC = typename FC_FWD_CPU::OUTPUT_SHAPE;
    using OCU = typename FC_FWD_CUDA::OUTPUT_SHAPE;
    ...
    std::cout << "Dense BF16 EVALUATE FP32(CPU) vs BF16(CUDA): " << diff << std::endl;
...
TEST(NN_LAYERS_DENSE_CUDA_BF16, FORWARD) {
    DEVICE_CPU device_cpu; DEVICE_CUDA device_cuda; rlt::init(device_cuda);
    RNG_CPU rng_cpu; rlt::malloc(device_cpu, rng_cpu); rlt::init(device_cpu, rng_cpu, 0);
    const char *dp = RL_TOOLS_MACRO_TO_STR(RL_TOOLS_TEST_DATA_PATH);
    auto file = HighFive::File(std::string(dp) + "/resnet18_test_data.h5", HighFive::File::ReadOnly);
    ...
    auto fc_group = rlt::get_group(device_cpu, lg, "11");
    ASSERT_TRUE(rlt::load(device_cpu, lc, fc_group));
    FC_GRAD_CUDA lcu; typename FC_GRAD_CUDA::template Buffer<true> bcu;
    rlt::malloc(device_cuda, lcu); rlt::malloc(device_cuda, bcu);
    rlt::copy(device_cpu, device_cuda, lc, lcu);
    rlt::Tensor<rlt::tensor::Specification<T, TI, INPUT_SHAPE>> ic; rlt::malloc(device_cpu, ic);
    auto tg = rlt::get_group(device_cpu, file, "test_data");
    ...
    using OC = typename FC_GRAD_CPU::OUTPUT_SHAPE;
    rlt::Tensor<rlt::tensor::Specification<T, TI, OC>> och; rlt::malloc(device_cpu, och);
    rlt::copy(device_cuda, device_cpu, rlt::output(device_cuda, lcu), och);
    T diff = rlt::abs_diff(device_cpu, rlt::output(device_cpu, lc), och) / decltype(och)::SPEC::SIZE;
    std::cout << "Dense BF16 FORWARD FP32(CPU) vs BF16(CUDA): " << diff << std::endl;
    EXPECT_LT(diff, FWD_EPSILON);
    rlt::free(device_cpu, lc); rlt::free(device_cpu, bc);
    rlt::free(device_cuda, lcu); rlt::free(device_cuda, bcu);
    rlt::free(device_cpu, ic); rlt::free(device_cuda, icu);
...
TEST(NN_LAYERS_DENSE_CUDA_BF16, BACKWARD) {
    DEVICE_CPU device_cpu; DEVICE_CUDA device_cuda; rlt::init(device_cuda);
    RNG_CPU rng_cpu; rlt::malloc(device_cpu, rng_cpu); rlt::init(device_cpu, rng_cpu, 0);
    const char *dp = RL_TOOLS_MACRO_TO_STR(RL_TOOLS_TEST_DATA_PATH);
    auto file = HighFive::File(std::string(dp) + "/resnet18_test_data.h5", HighFive::File::ReadOnly);
    ...
    rlt::forward(device_cuda, lcu, icu, bcu, rng_cuda, eval_mode);
    using OC = typename FC_GRAD_CPU::OUTPUT_SHAPE;
    using OCU = typename FC_GRAD_CUDA::OUTPUT_SHAPE;
    ...
    rlt::Tensor<rlt::tensor::Specification<T_BF16, TI_CUDA, INPUT_SHAPE_CUDA>> dicu; rlt::malloc(device_cuda, dicu);
