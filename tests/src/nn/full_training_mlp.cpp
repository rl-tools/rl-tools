    ...
    for(int epoch_i=0; epoch_i < n_epochs; epoch_i++){
        ...
        for (int batch_i=0; batch_i < n_iter; batch_i++){
            ...
            for (int sample_i=0; sample_i < batch_size; sample_i++){
                ...
                lic::Matrix<lic::matrix::Specification<T, DEVICE::index_t, 1, INPUT_DIM, lic::matrix::layouts::RowMajorAlignment<typename DEVICE::index_t>>> input_matrix;
                input_matrix._data = input;
                lic::Matrix<lic::matrix::Specification<T, DEVICE::index_t, 1, OUTPUT_DIM, lic::matrix::layouts::RowMajorAlignment<typename DEVICE::index_t>>> output_matrix;
                output_matrix._data = output;
                lic::forward(device, network, input_matrix);
                T d_loss_d_output[OUTPUT_DIM];
                lic::Matrix<lic::matrix::Specification<T, DEVICE::index_t, 1, OUTPUT_DIM, lic::matrix::layouts::RowMajorAlignment<typename DEVICE::index_t>>> d_loss_d_output_matrix;
                ...
                T d_input[INPUT_DIM];
                lic::Matrix<lic::matrix::Specification<T, DEVICE::index_t, 1, INPUT_DIM, lic::matrix::layouts::RowMajorAlignment<typename DEVICE::index_t>>> d_input_matrix;
