...
// ABI boundary between the hypert core module and the JIT-compiled renderer libraries.
// Both sides are compiled from the same headers with the same compiler in the same build
// directory, so passing rl_tools scene types by pointer is safe. Everything renderer-spec
// dependent is hidden behind the vtable; buffer exchange uses raw pointers sized by config().
...
namespace hypert {
    struct Config {
        ...
        int output_mode;                // rl_tools::rendering::raytracing::OutputMode value
