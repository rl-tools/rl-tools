...
class RendererConfig:
    ...
    def canonical(self):
        # must match build_config_string() in cpp/renderer_impl.cpp exactly
        return (
    ...
    def config_line(self):
        return (
            f"{self.key()} {self.width} {self.height} {self.num_cameras} {self.num_probes} "
            f"{self.shading} {self.output_mode} {self.motion_blur_samples} {self.anti_aliasing_grid} "
            f"{self.num_overlays} {self.max_overlay_instances} {self.max_overlays_per_camera} "
            f"{1 if self.semantic_segmentation else 0}"
        )
