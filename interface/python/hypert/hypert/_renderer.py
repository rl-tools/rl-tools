...
from ._build import ensure_renderer_library, load_core
from ._config import (
    OUTPUT_MODE,
    RENDER_PHASE,
    RENDER_TARGET,
...
def _from_dlpack(value):
    ...
    if not isinstance(value, np.ndarray) and hasattr(value, "__dlpack__"):
        try:
            ...
            raise ValueError(
                "hypert: input tensor could not be viewed as CPU memory (device-resident "
                "inputs are not supported yet — move it to the CPU first)"
            ) from error
    return value
...
class Renderer:
    ...
    combination compiles a renderer library into the hypert cache, subsequent uses load
    the cached library directly.
    """
