...
def make_quad(center_x, half_size=1.0, color=(1.0, 0.0, 0.0)):
    ...
    return hypert.Mesh(vertices, indices, color=color)


def make_scene(distance=2.0):
    scene = hypert.Scene()
    wall = hypert.Object(name="wall")
    wall.add_mesh(make_quad(distance))
    scene.add_object(wall)
    scene.add_light(hypert.SceneLight.directional(direction=(1.0, 0.0, 0.0), color=(1.0, 1.0, 1.0)))
    return scene
...
def test_depth():
    distance = 2.0
    renderer = hypert.Renderer(width=64, height=64, num_cameras=1, output="depth", shading="low")
...
def test_rgb():
    renderer = hypert.Renderer(width=32, height=32, num_cameras=2, output="rgb", shading="low")
...
def make_quad_span(x, y_low, y_high, z_half=5.0):
    ...
    indices = np.array([[0, 2, 1], [0, 3, 2]], dtype=np.int32)
    return hypert.Mesh(vertices, indices)
...
def test_segmentation():
    ...
    for index, (y_low, y_high) in enumerate(((-5.0, -0.05), (0.05, 5.0))):
        wall = hypert.Object(name=f"wall_{index}")
        wall.add_mesh(make_quad_span(2.0, y_low, y_high))
        scene.add_object(wall)
    renderer = hypert.Renderer(width=32, height=32, num_cameras=1, output="segmentation", shading="low")
    renderer.init(scene)
    look_forward(renderer)
...
def test_collision_probes():
    renderer = hypert.Renderer(width=16, height=16, num_cameras=1, num_probes=4, output="depth", shading="low")
...
def test_jit_cache_reuse():
    ...
    library = hypert.build_dir() / "jit" / f"hypert_renderer_{config.key()}.so"
    renderer = hypert.Renderer(width=16, height=16, num_cameras=1, output="depth", shading="low")
    assert library.exists()
    modification_time = library.stat().st_mtime
    renderer_again = hypert.Renderer(width=16, height=16, num_cameras=1, output="depth", shading="low")
...
def test_dlpack_export():
    renderer = hypert.Renderer(width=16, height=16, num_cameras=1, output="rgbd", shading="low")
    renderer.init(make_scene(2.0))
    look_forward(renderer)
...
def test_glb_roundtrip(tmp_path):
    ...
    renderer = hypert.Renderer(width=32, height=32, num_cameras=1, output="rgbd", shading="medium")
    renderer.init(scene)
    look_forward(renderer)
    renderer.render("rgb_depth")
    frame = renderer.frame()
...
def test_glb_assembly_names(tmp_path):
    glb = tmp_path / "quad.glb"
    write_minimal_glb(glb)
    assembly = hypert.load_assembly(glb, shading="low")
...
GLB = Path(os.environ.get("HYPERT_RLTOOLS_ROOT", Path(__file__).resolve().parents[4])) / "tests" / "data" / "ProcTHOR-Train-1.glb"


@pytest.mark.skipif(not GLB.exists(), reason="ProcTHOR test scene not available")
def test_glb_scene():
    scene = hypert.load_scene(GLB, shading="high")
    assert scene.num_objects >= 1
    assert scene.num_instances >= 1
    renderer = hypert.Renderer(width=64, height=48, num_cameras=1, output="rgbd", shading="high")
    renderer.init(scene)
