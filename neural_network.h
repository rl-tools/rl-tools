...
template <typename T>
T erf_d(T x){
    return (T)2 / std::sqrt((T)M_PI) * std::exp(-x * x);
}
...
FUNCTION_PLACEMENT void update_layer(LayerBackwardAdam<T, INPUT_DIM, OUTPUT_DIM, FN, PARAMETERS>& layer, T first_order_moment_bias_correction, T second_order_moment_bias_correction, const uint32_t batch_size) {
    polyak_update_matrix<T, OUTPUT_DIM, INPUT_DIM>(layer.d_weights_first_order_moment, layer.d_weights, PARAMETERS::BETA_1);
    polyak_update       <T, OUTPUT_DIM>           (layer. d_biases_first_order_moment, layer.d_biases , PARAMETERS::BETA_1);

    polyak_update_squared_matrix<T, OUTPUT_DIM, INPUT_DIM>(layer.d_weights_second_order_moment, layer.d_weights, PARAMETERS::BETA_2);
    polyak_update_squared       <T, OUTPUT_DIM>           (layer. d_biases_second_order_moment, layer.d_biases , PARAMETERS::BETA_2);
