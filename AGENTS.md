...
30. Episode accounting is integral to the batch-first `OnPolicyRunner<on_policy_runner::Specification<...>>`. The runner owns ongoing counters/returns, the current reset mask, and the current completion event; `on_policy_runner::Buffer` owns transient next states/actions/rewards/termination flags; the rollout `Dataset` owns transition flags and completed-episode records. `collect` runs `prologue` once (observation row 0 and its reset column), then `interlude` (recurrent reset, policy evaluation, action sampling) and `epilogue` (step, reward, termination, accounting, autoreset, observation row t+1) per step. External boundaries use the immediate `reset(device, runner, environment[, mask], rng)` operation. Semantics are fixed: same-step autoreset, no final observation, `terminated` implies `truncated`, and forced boundaries have reason `FORCED`. Rendering is not part of the runner or generic Environment API; visual environments perform or cache it inside their `step`/`observe` implementation. Ordinary scalar environments use the independent batch adapter in `rl/environments/batch/`, and recurrent policies consume `runner.reset` through `Mode<mode::sequential::ResetMask>`.
