...
30. Episode bookkeeping is part of the batch-first `OnPolicyRunner<on_policy_runner::Specification<...>>`: the runner owns its counters, returns, reset/termination/truncation masks, end reasons, and finished-episode outputs on the collection device. Drivers call `begin_step(device, runner, environment, rng)` and `end_step(device, runner, environment, rewards, rng)` rather than reimplementing lifecycle logic; `force_reset(device, runner[, mask])` handles external resets and `record(log, runner, step_i)` writes an `on_policy_runner::EpisodeLog`. Semantics are fixed: same-step autoreset (the observation after a reset is the first of the new episode, no final observation), `terminated` implies `truncated`, `reset` is truncation delayed by one step, forced resets use reason `FORCED`, and `SYNCHRONIZED` gives deterministic all-or-none resets on CPU and CUDA. The runner accepts any environment exposing tensor-batched verbs over `INSTANCES`; ordinary scalar environments use the independent batch adapter in `rl/environments/batch/`. The environment object is owned by the driver, persistent rollout and episode data by `OnPolicyRunner`, and `next_states`/sampled actions/rewards by `on_policy_runner::Buffer`. `init` applies the initial reset and render; `collect` runs `prologue` (observe row 0, record reset) and then `interlude` (recurrent reset from `runner.reset`, batched policy evaluation, Gaussian action sampling) plus `epilogue` (step, reward, episode accounting, dataset record, autoreset, render, observe row t+1). Recurrent policies consume `runner.reset` through `Mode<mode::sequential::ResetMask>` (rank-1 bool tensor or 1×N matrix, device-generic).
