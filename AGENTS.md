...
Canonical renderings live in the separate test-data repository under `tests/data/rendering_raytracing_golden/`: the existing ProcTHOR suite uses `procthor_static_scene/` (per-pose `<case>.png`/`<case>_depth.bin`/`probes.bin` plus a single per-pose `normals.png` and `segmentation.bin`/`segmentation.png` — normals and segmentation are single-sample and shading/AA/MB-independent, so one golden covers all cases; the geometry case loads the scene as one instance per GLB root node so segmentation ids are pinned by node order, while the RGB cases keep the welded single-object scene), and synthetic overlay cases use `overlay/<scenario>/<state>/<view>/{rgb.png,depth.bin,depth.png,segmentation.bin,segmentation.png,normals.png}`. Overlay PNGs are 2x2 grids in camera order `0,1 / 2,3`; versioned depth and segmentation binaries contain `[4,64,64]` (overlay) or `[1,64,64]` per pose (ProcTHOR). `rgb.png`, the `.bin` files, and `normals.png` are the machine-compared targets (`normals.png` stores the float normals through the pinned `round((clamp(n,-1,1)*0.5+0.5)*255)` encoding — miss pixels are exactly `(128,128,128)`, which no unit normal can produce); `segmentation.png` stores ids through the pinned false-color encoding in `golden_io.h` and is validated against `segmentation.bin` (by the corpus test for overlay, by the backend comparator for ProcTHOR), while `depth.png` is an advisory review image only (its normalization depends on the generation-time camera radius). On a fresh checkout, `./tests/download_data.sh --raytracing-goldens` downloads only this corpus. Configure `RL_TOOLS_REQUIRE_RAYTRACING_GOLDENS=ON` when missing manifests or LFS objects must fail instead of skip.
